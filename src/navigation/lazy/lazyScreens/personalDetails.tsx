...
//@ts-ignore
const PersonalDetailsScreen = lazy(() => {
  //looks like cookies are the only way to pass auth with lazy loaded chunks

  return import(
    /* webpackChunkName: "cs222rt" */ "src/pages/settings/personalDetails/personalDetails"
  ).catch(() => {
    store.dispatch(invalidateAuth());
  });
});

export const LazyPersonalDetails = ({ navigation }: { navigation: any }) => {
  return (
    <Suspense
      fallback={
        <View>
    ...
    >
      <PersonalDetailsScreen navigation={navigation} />
    </Suspense>
  );
};

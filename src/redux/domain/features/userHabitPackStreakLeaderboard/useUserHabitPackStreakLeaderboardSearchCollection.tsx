...
import { clearUserHabitPackStreakLeaderboardItems, userHabitPackStreakLeaderboardsSelector, searchUserHabitPackStreakLeaderboards, userHabitPackStreakLeaderboardsLoading } from "./collection-slice";
...
export const useUserHabitPackStreakLeaderboardsSearchCollection = (userhabitpack?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadUserHabitPackStreakLeaderboards = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(userhabitpack) dispatch(searchUserHabitPackStreakLeaderboards({ userhabitpack: userhabitpack, search: search, offset: offset || 0, limit: limit, filter: filter }));
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchUserHabitPackStreakLeaderboards({
    ...
    useEffect(() => {
        if (!loading && (!userhabitpackstreakleaderboards ||
        userhabitpackstreakleaderboards.length === 0) && 
        (!initialised || initialisedSubKey.current !== userhabitpack)
          && userhabitpack) {
            setInitialised(true);
            initialisedSubKey.current = userhabitpack;
            loadUserHabitPackStreakLeaderboards({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        userhabitpackstreakleaderboards, 
    ...
    return {
        LoadMoreUserHabitPackStreakLeaderboardsSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadUserHabitPackStreakLeaderboards({
                offset: userhabitpackstreakleaderboards?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }

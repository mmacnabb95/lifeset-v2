...
import { clearRandomInspoQuoteItems, randomInspoQuotesSelector, searchRandomInspoQuotes, randomInspoQuotesLoading } from "./collection-slice";
...
export const useRandomInspoQuotesSearchCollection = ( initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadRandomInspoQuotes = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        dispatch(searchRandomInspoQuotes({ search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch]);
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchRandomInspoQuotes({
    ...
    useEffect(() => {
        if (!loading && (!randominspoquotes ||
        randominspoquotes.length === 0) && 
        (!initialised )
         ) {
            setInitialised(true);
            
            loadRandomInspoQuotes({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        randominspoquotes, 
    ...
    return {
        LoadMoreRandomInspoQuotesSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadRandomInspoQuotes({
                offset: randominspoquotes?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }

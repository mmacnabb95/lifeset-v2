...
import { clearBenefitItems, benefitsSelector, searchBenefits, benefitsLoading } from "./collection-slice";
...
export const useBenefitsSearchCollection = (company?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadBenefits = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(company) dispatch(searchBenefits({ company: company, search: search, offset: offset || 0, limit: limit, filter: filter }));
    ...
    useEffect(() => {
        ...
        (!initialised || initialisedSubKey.current !== company)
          && company) {
            ...
            loadBenefits({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        benefits, 
        loadBenefits, 

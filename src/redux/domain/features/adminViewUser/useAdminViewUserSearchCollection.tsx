...
import { clearAdminViewUserItems, adminViewUsersSelector, searchAdminViewUsers, adminViewUsersLoading } from "./collection-slice";
...
export const useAdminViewUsersSearchCollection = ( initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadAdminViewUsers = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        dispatch(searchAdminViewUsers({ search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch]);
    ...
    useEffect(() => {
        if (!loading && (!adminviewusers ||
        adminviewusers.length === 0) && 
        (!initialised )
         ) {
            setInitialised(true);
            
            loadAdminViewUsers({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        adminviewusers, 
        loadAdminViewUsers, 
    ...
    return {
        LoadMoreAdminViewUsersSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}

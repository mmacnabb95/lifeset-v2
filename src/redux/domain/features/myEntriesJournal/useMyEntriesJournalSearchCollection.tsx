...
import { clearMyEntriesJournalItems, myEntriesJournalsSelector, searchMyEntriesJournals, myEntriesJournalsLoading } from "./collection-slice";
...
export const useMyEntriesJournalsSearchCollection = (user?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadMyEntriesJournals = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(user) dispatch(searchMyEntriesJournals({ user: user, search: search, offset: offset || 0, limit: limit, filter: filter }));
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchMyEntriesJournals({
            user: user, 
    ...
    useEffect(() => {
        if (!loading && (!myentriesjournals ||
        myentriesjournals.length === 0) && 
        (!initialised || initialisedSubKey.current !== user)
          && user) {
            setInitialised(true);
            initialisedSubKey.current = user;
            loadMyEntriesJournals({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        myentriesjournals, 
    ...
    return {
        LoadMoreMyEntriesJournalsSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadMyEntriesJournals({
                offset: myentriesjournals?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }

...
import { clearHabitPackHabitScheduleItems, habitPackHabitSchedulesSelector, searchHabitPackHabitSchedules, habitPackHabitSchedulesLoading } from "./collection-slice";
...
export const useHabitPackHabitSchedulesSearchCollection = (userhabitpackhabit?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    const dispatch = useDispatch();
    const cmsStyles = useCmsStyles();
    const [initialised, setInitialised] = useState(false);
    const initialisedSubKey = useRef(userhabitpackhabit);
    
    const [searchParamsLoaded, setSearchParamsLoaded] = useState(false);
    const [textFilter, setTextFilter] = useState<string | undefined>();

    
    const habitpackhabitschedules = useSelector(habitPackHabitSchedulesSelector(userhabitpackhabit)); 
    ...
    const loadHabitPackHabitSchedules = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(userhabitpackhabit) dispatch(searchHabitPackHabitSchedules({ userhabitpackhabit: userhabitpackhabit, search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch, userhabitpackhabit]);
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchHabitPackHabitSchedules({
            userhabitpackhabit: userhabitpackhabit, 
    ...
    useEffect(() => {
        if (!loading && (!habitpackhabitschedules ||
        habitpackhabitschedules.length === 0) && 
        (!initialised || initialisedSubKey.current !== userhabitpackhabit)
          && userhabitpackhabit) {
            setInitialised(true);
            initialisedSubKey.current = userhabitpackhabit;
            loadHabitPackHabitSchedules({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        habitpackhabitschedules, 
        loadHabitPackHabitSchedules, 
        initialised, userhabitpackhabit
    ]);
    ...
    return {
        LoadMoreHabitPackHabitSchedulesSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadHabitPackHabitSchedules({
                offset: habitpackhabitschedules?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }

...
import { clearUserHabitPackItems, userHabitPacksSelector, searchUserHabitPacks, userHabitPacksLoading } from "./collection-slice";
...
export const useUserHabitPacksSearchCollection = (user?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadUserHabitPacks = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(user) dispatch(searchUserHabitPacks({ user: user, search: search, offset: offset || 0, limit: limit, filter: filter }));
    ...
    useEffect(() => {
        if (!loading && (!userhabitpacks ||
        userhabitpacks.length === 0) && 
        (!initialised || initialisedSubKey.current !== user)
          && user) {
            setInitialised(true);
            initialisedSubKey.current = user;
            loadUserHabitPacks({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        userhabitpacks, 
        loadUserHabitPacks, 
    ...
    const Filters = ({
          ...
          return <ListFilter
                      ...
                      doSearch={doSearch}
                      reSearch={reSearch}
                      clearingThunk={clearUserHabitPackItems}
    ...
    return {
        LoadMoreUserHabitPacksSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}

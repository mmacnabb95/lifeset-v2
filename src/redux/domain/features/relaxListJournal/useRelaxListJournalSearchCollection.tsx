...
import { clearRelaxListJournalItems, relaxListJournalsSelector, searchRelaxListJournals, relaxListJournalsLoading } from "./collection-slice";
...
export const useRelaxListJournalsSearchCollection = (user?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadRelaxListJournals = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(user) dispatch(searchRelaxListJournals({ user: user, search: search, offset: offset || 0, limit: limit, filter: filter }));
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchRelaxListJournals({
            user: user, 
    ...
    useEffect(() => {
        if (!loading && (!relaxlistjournals ||
        relaxlistjournals.length === 0) && 
        (!initialised || initialisedSubKey.current !== user)
          && user) {
            setInitialised(true);
            initialisedSubKey.current = user;
            loadRelaxListJournals({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        relaxlistjournals, 
    ...
    return {
        LoadMoreRelaxListJournalsSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadRelaxListJournals({
                offset: relaxlistjournals?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }

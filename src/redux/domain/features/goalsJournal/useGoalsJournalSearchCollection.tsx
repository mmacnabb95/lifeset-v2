...
import { clearGoalsJournalItems, goalsJournalsSelector, searchGoalsJournals, goalsJournalsLoading } from "./collection-slice";
...
export const useGoalsJournalsSearchCollection = (user?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadGoalsJournals = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(user) dispatch(searchGoalsJournals({ user: user, search: search, offset: offset || 0, limit: limit, filter: filter }));
    ...
    useEffect(() => {
        if (!loading && (!goalsjournals ||
        goalsjournals.length === 0) && 
        (!initialised || initialisedSubKey.current !== user)
          && user) {
            setInitialised(true);
            initialisedSubKey.current = user;
            loadGoalsJournals({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        goalsjournals, 
        loadGoalsJournals, 
    ...
    return {
        LoadMoreGoalsJournalsSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}

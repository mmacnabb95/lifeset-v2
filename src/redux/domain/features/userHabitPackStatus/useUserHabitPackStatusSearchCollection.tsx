...
import { clearUserHabitPackStatusItems, userHabitPackStatussSelector, searchUserHabitPackStatuss, userHabitPackStatussLoading } from "./collection-slice";
...
export const useUserHabitPackStatussSearchCollection = ( initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const toggleFilter = (fieldName: string, filterTypeId?: number) => {
        ...
        dispatch(clearUserHabitPackStatusItems());
        reSearch();
    };
    ...
    const loadUserHabitPackStatuss = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        dispatch(searchUserHabitPackStatuss({ search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch]);
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchUserHabitPackStatuss({
    ...
    useEffect(() => {
        if (!loading && (!userhabitpackstatuss ||
        userhabitpackstatuss.length === 0) && 
        (!initialised )
         ) {
            setInitialised(true);
            
            loadUserHabitPackStatuss({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
    ...
    return {
        LoadMoreUserHabitPackStatussSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadUserHabitPackStatuss({
                offset: userhabitpackstatuss?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }

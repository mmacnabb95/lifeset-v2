...
import { clearLanguageItems, languagesSelector, searchLanguages, languagesLoading } from "./collection-slice";
...
export const useLanguagesSearchCollection = ( initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadLanguages = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        dispatch(searchLanguages({ search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch]);
    ...
    useEffect(() => {
        ...
        (!initialised )
            ...
            loadLanguages({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        languages, 
        loadLanguages, 

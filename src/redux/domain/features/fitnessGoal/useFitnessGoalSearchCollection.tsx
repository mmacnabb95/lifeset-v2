...
import { clearFitnessGoalItems, fitnessGoalsSelector, searchFitnessGoals, fitnessGoalsLoading } from "./collection-slice";
...
export const useFitnessGoalsSearchCollection = ( initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadFitnessGoals = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        dispatch(searchFitnessGoals({ search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch]);
    ...
    useEffect(() => {
        ...
        (!initialised )
            ...
            loadFitnessGoals({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        fitnessgoals, 
        loadFitnessGoals, 
    ...
    return {
        LoadMoreFitnessGoalsSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}

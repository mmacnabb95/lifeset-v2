...
import { clearInspoQuoteItems, inspoQuotesSelector, searchInspoQuotes, inspoQuotesLoading } from "./collection-slice";
...
export const useInspoQuotesSearchCollection = ( initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadInspoQuotes = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        dispatch(searchInspoQuotes({ search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch]);
    ...
    useEffect(() => {
        if (!loading && (!inspoquotes ||
        inspoquotes.length === 0) && 
        (!initialised )
         ) {
            setInitialised(true);
            
            loadInspoQuotes({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        inspoquotes, 
        loadInspoQuotes, 
    ...
    return {
        LoadMoreInspoQuotesSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}

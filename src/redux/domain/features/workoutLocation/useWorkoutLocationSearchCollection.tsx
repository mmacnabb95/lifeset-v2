...
import { clearWorkoutLocationItems, workoutLocationsSelector, searchWorkoutLocations, workoutLocationsLoading } from "./collection-slice";
...
export const useWorkoutLocationsSearchCollection = ( initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadWorkoutLocations = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        dispatch(searchWorkoutLocations({ search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch]);
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchWorkoutLocations({
    ...
    useEffect(() => {
        if (!loading && (!workoutlocations ||
        workoutlocations.length === 0) && 
        (!initialised )
         ) {
            setInitialised(true);
            
            loadWorkoutLocations({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        workoutlocations, 
    ...
    return {
        LoadMoreWorkoutLocationsSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadWorkoutLocations({
                offset: workoutlocations?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }

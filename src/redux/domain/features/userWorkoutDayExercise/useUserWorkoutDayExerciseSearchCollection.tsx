...
import { clearUserWorkoutDayExerciseItems, userWorkoutDayExercisesSelector, searchUserWorkoutDayExercises, userWorkoutDayExercisesLoading } from "./collection-slice";
...
export const useUserWorkoutDayExercisesSearchCollection = (userworkout?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadUserWorkoutDayExercises = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(userworkout) dispatch(searchUserWorkoutDayExercises({ userworkout: userworkout, search: search, offset: offset || 0, limit: limit, filter: filter }));
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchUserWorkoutDayExercises({
    ...
    useEffect(() => {
        if (!loading && (!userworkoutdayexercises ||
        userworkoutdayexercises.length === 0) && 
        (!initialised || initialisedSubKey.current !== userworkout)
          && userworkout) {
            setInitialised(true);
            initialisedSubKey.current = userworkout;
            loadUserWorkoutDayExercises({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
    ...
    return {
        LoadMoreUserWorkoutDayExercisesSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadUserWorkoutDayExercises({
                offset: userworkoutdayexercises?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }

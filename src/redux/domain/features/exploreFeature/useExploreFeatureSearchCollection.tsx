...
import { clearExploreFeatureItems, exploreFeaturesSelector, searchExploreFeatures, exploreFeaturesLoading } from "./collection-slice";
...
export const useExploreFeaturesSearchCollection = (user?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadExploreFeatures = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(user) dispatch(searchExploreFeatures({ user: user, search: search, offset: offset || 0, limit: limit, filter: filter }));
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchExploreFeatures({
            user: user, 
    ...
    useEffect(() => {
        if (!loading && (!explorefeatures ||
        explorefeatures.length === 0) && 
        (!initialised || initialisedSubKey.current !== user)
          && user) {
            setInitialised(true);
            initialisedSubKey.current = user;
            loadExploreFeatures({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        explorefeatures, 
        loadExploreFeatures, 
    ...
    return {
        LoadMoreExploreFeaturesSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}

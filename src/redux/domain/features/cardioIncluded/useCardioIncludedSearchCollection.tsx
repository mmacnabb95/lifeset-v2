...
import { clearCardioIncludedItems, cardioIncludedsSelector, searchCardioIncludeds, cardioIncludedsLoading } from "./collection-slice";
...
export const useCardioIncludedsSearchCollection = ( initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadCardioIncludeds = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        dispatch(searchCardioIncludeds({ search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch]);
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchCardioIncludeds({
    ...
    useEffect(() => {
        if (!loading && (!cardioincludeds ||
        cardioincludeds.length === 0) && 
        (!initialised )
         ) {
            setInitialised(true);
            
            loadCardioIncludeds({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        cardioincludeds, 
        loadCardioIncludeds, 
    ...
    return {
        LoadMoreCardioIncludedsSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}

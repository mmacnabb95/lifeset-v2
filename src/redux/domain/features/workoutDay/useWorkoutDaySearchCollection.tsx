...
import { clearWorkoutDayItems, workoutDaysSelector, searchWorkoutDays, workoutDaysLoading } from "./collection-slice";
...
export const useWorkoutDaysSearchCollection = (workout?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    const dispatch = useDispatch();
    const cmsStyles = useCmsStyles();
    const [initialised, setInitialised] = useState(false);
    const initialisedSubKey = useRef(workout);
    ...
    const loadWorkoutDays = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(workout) dispatch(searchWorkoutDays({ workout: workout, search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch, workout]);
    ...
    useEffect(() => {
        if (!loading && (!workoutdays ||
        workoutdays.length === 0) && 
        (!initialised || initialisedSubKey.current !== workout)
          && workout) {
            setInitialised(true);
            initialisedSubKey.current = workout;
            loadWorkoutDays({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        workoutdays, 
        loadWorkoutDays, 
    ...
    return {
        LoadMoreWorkoutDaysSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}

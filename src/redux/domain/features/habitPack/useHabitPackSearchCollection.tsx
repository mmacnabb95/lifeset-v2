...
import { clearHabitPackItems, habitPacksSelector, searchHabitPacks, habitPacksLoading } from "./collection-slice";
...
export const useHabitPacksSearchCollection = (company?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadHabitPacks = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(company) dispatch(searchHabitPacks({ company: company, search: search, offset: offset || 0, limit: limit, filter: filter }));
    ...
    useEffect(() => {
        if (!loading && (!habitpacks ||
        habitpacks.length === 0) && 
        (!initialised || initialisedSubKey.current !== company)
          && company) {
            setInitialised(true);
            initialisedSubKey.current = company;
            loadHabitPacks({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        habitpacks, 
        loadHabitPacks, 
    ...
    return {
        LoadMoreHabitPacksSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}

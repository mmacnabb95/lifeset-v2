...
import { clearUserWorkoutExerciseSetItems, userWorkoutExerciseSetsSelector, searchUserWorkoutExerciseSets, userWorkoutExerciseSetsLoading } from "./collection-slice";
...
export const useUserWorkoutExerciseSetsSearchCollection = (userworkout?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    const dispatch = useDispatch();
    const cmsStyles = useCmsStyles();
    const [initialised, setInitialised] = useState(false);
    const initialisedSubKey = useRef(userworkout);
    ...
    const loadUserWorkoutExerciseSets = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(userworkout) dispatch(searchUserWorkoutExerciseSets({ userworkout: userworkout, search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch, userworkout]);
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchUserWorkoutExerciseSets({
    ...
    useEffect(() => {
        if (!loading && (!userworkoutexercisesets ||
        userworkoutexercisesets.length === 0) && 
        (!initialised || initialisedSubKey.current !== userworkout)
          && userworkout) {
            setInitialised(true);
            initialisedSubKey.current = userworkout;
            loadUserWorkoutExerciseSets({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        userworkoutexercisesets, 
        loadUserWorkoutExerciseSets, 
        initialised, userworkout
    ]);
    ...
    return {
        LoadMoreUserWorkoutExerciseSetsSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadUserWorkoutExerciseSets({
                offset: userworkoutexercisesets?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }

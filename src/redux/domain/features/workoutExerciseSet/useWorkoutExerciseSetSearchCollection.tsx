...
import { clearWorkoutExerciseSetItems, workoutExerciseSetsSelector, searchWorkoutExerciseSets, workoutExerciseSetsLoading } from "./collection-slice";
...
export const useWorkoutExerciseSetsSearchCollection = (workoutdayexercise?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    const dispatch = useDispatch();
    const cmsStyles = useCmsStyles();
    const [initialised, setInitialised] = useState(false);
    const initialisedSubKey = useRef(workoutdayexercise);
    ...
    const loadWorkoutExerciseSets = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(workoutdayexercise) dispatch(searchWorkoutExerciseSets({ workoutdayexercise: workoutdayexercise, search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch, workoutdayexercise]);
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchWorkoutExerciseSets({
    ...
    useEffect(() => {
        if (!loading && (!workoutexercisesets ||
        workoutexercisesets.length === 0) && 
        (!initialised || initialisedSubKey.current !== workoutdayexercise)
          && workoutdayexercise) {
            setInitialised(true);
            initialisedSubKey.current = workoutdayexercise;
            loadWorkoutExerciseSets({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        workoutexercisesets, 
        loadWorkoutExerciseSets, 
        initialised, workoutdayexercise
    ]);
    ...
    return {
        LoadMoreWorkoutExerciseSetsSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadWorkoutExerciseSets({
                offset: workoutexercisesets?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }

...
import { clearCompanyUserItems, companyUsersSelector, searchCompanyUsers, companyUsersLoading } from "./collection-slice";
...
export const useCompanyUsersSearchCollection = (company?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadCompanyUsers = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(company) dispatch(searchCompanyUsers({ company: company, search: search, offset: offset || 0, limit: limit, filter: filter }));
    ...
    useEffect(() => {
        ...
        (!initialised || initialisedSubKey.current !== company)
          && company) {
            ...
            loadCompanyUsers({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        companyusers, 
        loadCompanyUsers, 
    ...
    return {
        LoadMoreCompanyUsersSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}

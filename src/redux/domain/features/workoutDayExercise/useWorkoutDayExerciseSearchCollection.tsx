...
import { clearWorkoutDayExerciseItems, workoutDayExercisesSelector, searchWorkoutDayExercises, workoutDayExercisesLoading } from "./collection-slice";
...
export const useWorkoutDayExercisesSearchCollection = (workoutday?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    const dispatch = useDispatch();
    const cmsStyles = useCmsStyles();
    const [initialised, setInitialised] = useState(false);
    const initialisedSubKey = useRef(workoutday);
    ...
    const loadWorkoutDayExercises = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(workoutday) dispatch(searchWorkoutDayExercises({ workoutday: workoutday, search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch, workoutday]);
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchWorkoutDayExercises({
    ...
    useEffect(() => {
        if (!loading && (!workoutdayexercises ||
        workoutdayexercises.length === 0) && 
        (!initialised || initialisedSubKey.current !== workoutday)
          && workoutday) {
            setInitialised(true);
            initialisedSubKey.current = workoutday;
            loadWorkoutDayExercises({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        workoutdayexercises, 
        loadWorkoutDayExercises, 
        initialised, workoutday
    ]);
    ...
    return {
        LoadMoreWorkoutDayExercisesSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadWorkoutDayExercises({
                offset: workoutdayexercises?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }

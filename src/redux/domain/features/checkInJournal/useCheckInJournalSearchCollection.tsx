...
import { clearCheckInJournalItems, checkInJournalsSelector, searchCheckInJournals, checkInJournalsLoading } from "./collection-slice";
...
export const useCheckInJournalsSearchCollection = (user?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const toggleFilter = (fieldName: string, filterTypeId?: number) => {
        ...
        dispatch(clearCheckInJournalItems());
        reSearch();
    };
    ...
    const loadCheckInJournals = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(user) dispatch(searchCheckInJournals({ user: user, search: search, offset: offset || 0, limit: limit, filter: filter }));
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchCheckInJournals({
            user: user, 
    ...
    useEffect(() => {
        if (!loading && (!checkinjournals ||
        checkinjournals.length === 0) && 
        (!initialised || initialisedSubKey.current !== user)
          && user) {
            setInitialised(true);
            initialisedSubKey.current = user;
            loadCheckInJournals({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        checkinjournals, 
        loadCheckInJournals, 
    ...
    return {
        LoadMoreCheckInJournalsSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}

...
import { clearHabitMediaKeyItems, habitMediaKeysSelector, searchHabitMediaKeys, habitMediaKeysLoading } from "./collection-slice";
...
export const useHabitMediaKeysSearchCollection = ( initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadHabitMediaKeys = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        dispatch(searchHabitMediaKeys({ search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch]);
    ...
    useEffect(() => {
        if (!loading && (!habitmediakeys ||
        habitmediakeys.length === 0) && 
        (!initialised )
         ) {
            setInitialised(true);
            
            loadHabitMediaKeys({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        habitmediakeys, 
        loadHabitMediaKeys, 
    ...
    return {
        LoadMoreHabitMediaKeysSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}

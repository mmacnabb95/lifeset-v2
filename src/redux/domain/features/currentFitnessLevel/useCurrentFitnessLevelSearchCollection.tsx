...
import { clearCurrentFitnessLevelItems, currentFitnessLevelsSelector, searchCurrentFitnessLevels, currentFitnessLevelsLoading } from "./collection-slice";
...
export const useCurrentFitnessLevelsSearchCollection = ( initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadCurrentFitnessLevels = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        dispatch(searchCurrentFitnessLevels({ search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch]);
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchCurrentFitnessLevels({
    ...
    useEffect(() => {
        if (!loading && (!currentfitnesslevels ||
        currentfitnesslevels.length === 0) && 
        (!initialised )
         ) {
            setInitialised(true);
            
            loadCurrentFitnessLevels({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
    ...
    return {
        LoadMoreCurrentFitnessLevelsSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadCurrentFitnessLevels({
                offset: currentfitnesslevels?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }

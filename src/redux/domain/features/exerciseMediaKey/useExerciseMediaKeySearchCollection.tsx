...
import { clearExerciseMediaKeyItems, exerciseMediaKeysSelector, searchExerciseMediaKeys, exerciseMediaKeysLoading } from "./collection-slice";
...
export const useExerciseMediaKeysSearchCollection = ( initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const toggleFilter = (fieldName: string, filterTypeId?: number) => {
        ...
        dispatch(clearExerciseMediaKeyItems());
        reSearch();
    };
    ...
    const loadExerciseMediaKeys = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        dispatch(searchExerciseMediaKeys({ search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch]);
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchExerciseMediaKeys({
    ...
    useEffect(() => {
        if (!loading && (!exercisemediakeys ||
        exercisemediakeys.length === 0) && 
        (!initialised )
         ) {
            setInitialised(true);
            
            loadExerciseMediaKeys({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        exercisemediakeys, 
    ...
    return {
        LoadMoreExerciseMediaKeysSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadExerciseMediaKeys({
                offset: exercisemediakeys?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }

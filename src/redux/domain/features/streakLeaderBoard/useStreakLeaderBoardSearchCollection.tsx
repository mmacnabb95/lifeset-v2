...
import { clearStreakLeaderBoardItems, streakLeaderBoardsSelector, searchStreakLeaderBoards, streakLeaderBoardsLoading } from "./collection-slice";
...
export const useStreakLeaderBoardsSearchCollection = (company?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    const dispatch = useDispatch();
    const cmsStyles = useCmsStyles();
    const [initialised, setInitialised] = useState(false);
    const initialisedSubKey = useRef(company);
    ...
    const loadStreakLeaderBoards = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(company) dispatch(searchStreakLeaderBoards({ company: company, search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch, company]);
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchStreakLeaderBoards({
            company: company, 
    ...
    useEffect(() => {
        if (!loading && (!streakleaderboards ||
        streakleaderboards.length === 0) && 
        (!initialised || initialisedSubKey.current !== company)
          && company) {
            setInitialised(true);
            initialisedSubKey.current = company;
            loadStreakLeaderBoards({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        streakleaderboards, 
    ...
    return {
        LoadMoreStreakLeaderBoardsSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadStreakLeaderBoards({
                offset: streakleaderboards?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }

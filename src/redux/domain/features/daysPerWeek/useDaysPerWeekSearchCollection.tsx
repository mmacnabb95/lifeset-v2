...
import { clearDaysPerWeekItems, daysPerWeeksSelector, searchDaysPerWeeks, daysPerWeeksLoading } from "./collection-slice";
...
export const useDaysPerWeeksSearchCollection = ( initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const toggleFilter = (fieldName: string, filterTypeId?: number) => {
        ...
        dispatch(clearDaysPerWeekItems());
        reSearch();
    };
    ...
    const loadDaysPerWeeks = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        dispatch(searchDaysPerWeeks({ search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch]);
    ...
    useEffect(() => {
        if (!loading && (!daysperweeks ||
        daysperweeks.length === 0) && 
        (!initialised )
         ) {
            setInitialised(true);
            
            loadDaysPerWeeks({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        daysperweeks, 
        loadDaysPerWeeks, 
    ...
    return {
        LoadMoreDaysPerWeeksSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}

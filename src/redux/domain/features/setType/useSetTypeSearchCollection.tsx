...
import { clearSetTypeItems, setTypesSelector, searchSetTypes, setTypesLoading } from "./collection-slice";
...
export const useSetTypesSearchCollection = ( initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const settypes = useSelector(setTypesSelector); 
    ...
    const loadSetTypes = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        dispatch(searchSetTypes({ search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch]);
    ...
    useEffect(() => {
        ...
        (!initialised )
            ...
            loadSetTypes({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        settypes, 
        loadSetTypes, 

...
import { clearExerciseItems, exercisesSelector, searchExercises, exercisesLoading } from "./collection-slice";
...
export const useExercisesSearchCollection = ( initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadExercises = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        dispatch(searchExercises({ search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch]);
    ...
    useEffect(() => {
        ...
        (!initialised )
            ...
            loadExercises({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        exercises, 
        loadExercises, 

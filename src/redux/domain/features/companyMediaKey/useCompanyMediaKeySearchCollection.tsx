...
import { clearCompanyMediaKeyItems, companyMediaKeysSelector, searchCompanyMediaKeys, companyMediaKeysLoading } from "./collection-slice";
...
export const useCompanyMediaKeysSearchCollection = ( initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadCompanyMediaKeys = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        dispatch(searchCompanyMediaKeys({ search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch]);
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchCompanyMediaKeys({
    ...
    useEffect(() => {
        if (!loading && (!companymediakeys ||
        companymediakeys.length === 0) && 
        (!initialised )
         ) {
            setInitialised(true);
            
            loadCompanyMediaKeys({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        companymediakeys, 
    ...
    return {
        LoadMoreCompanyMediaKeysSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadCompanyMediaKeys({
                offset: companymediakeys?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }

...
import { clearUserHabitPackUseItems, userHabitPackUsesSelector, searchUserHabitPackUses, userHabitPackUsesLoading } from "./collection-slice";
...
export const useUserHabitPackUsesSearchCollection = (user?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadUserHabitPackUses = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(user) dispatch(searchUserHabitPackUses({ user: user, search: search, offset: offset || 0, limit: limit, filter: filter }));
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchUserHabitPackUses({
            user: user, 
    ...
    useEffect(() => {
        if (!loading && (!userhabitpackuses ||
        userhabitpackuses.length === 0) && 
        (!initialised || initialisedSubKey.current !== user)
          && user) {
            setInitialised(true);
            initialisedSubKey.current = user;
            loadUserHabitPackUses({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        userhabitpackuses, 
    ...
    return {
        LoadMoreUserHabitPackUsesSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadUserHabitPackUses({
                offset: userhabitpackuses?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }

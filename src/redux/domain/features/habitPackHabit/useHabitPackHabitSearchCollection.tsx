...
import { clearHabitPackHabitItems, habitPackHabitsSelector, searchHabitPackHabits, habitPackHabitsLoading } from "./collection-slice";
...
export const useHabitPackHabitsSearchCollection = (habitpack?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    const dispatch = useDispatch();
    const cmsStyles = useCmsStyles();
    const [initialised, setInitialised] = useState(false);
    const initialisedSubKey = useRef(habitpack);
    ...
    const loadHabitPackHabits = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(habitpack) dispatch(searchHabitPackHabits({ habitpack: habitpack, search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch, habitpack]);
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchHabitPackHabits({
            habitpack: habitpack, 
    ...
    useEffect(() => {
        if (!loading && (!habitpackhabits ||
        habitpackhabits.length === 0) && 
        (!initialised || initialisedSubKey.current !== habitpack)
          && habitpack) {
            setInitialised(true);
            initialisedSubKey.current = habitpack;
            loadHabitPackHabits({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        habitpackhabits, 
        loadHabitPackHabits, 
        initialised, habitpack
    ]);
    ...
    return {
        LoadMoreHabitPackHabitsSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}

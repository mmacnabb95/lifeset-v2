...
import { clearClientEnvironmentItems, clientEnvironmentsSelector, searchClientEnvironments, clientEnvironmentsLoading } from "./collection-slice";
...
export const useClientEnvironmentsSearchCollection = ( initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const toggleFilter = (fieldName: string, filterTypeId?: number) => {
        ...
        dispatch(clearClientEnvironmentItems());
        reSearch();
    };
    ...
    const loadClientEnvironments = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        dispatch(searchClientEnvironments({ search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch]);
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchClientEnvironments({
    ...
    useEffect(() => {
        if (!loading && (!clientenvironments ||
        clientenvironments.length === 0) && 
        (!initialised )
         ) {
            setInitialised(true);
            
            loadClientEnvironments({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        clientenvironments, 
    ...
    return {
        LoadMoreClientEnvironmentsSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadClientEnvironments({
                offset: clientenvironments?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }

...
import { clearAllTimeStreakItems, allTimeStreaksSelector, searchAllTimeStreaks, allTimeStreaksLoading } from "./collection-slice";
...
export const useAllTimeStreaksSearchCollection = (user?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadAllTimeStreaks = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(user) dispatch(searchAllTimeStreaks({ user: user, search: search, offset: offset || 0, limit: limit, filter: filter }));
    ...
    return {
        LoadMoreAllTimeStreaksSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}

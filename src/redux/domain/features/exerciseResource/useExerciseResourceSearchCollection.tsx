...
import { clearExerciseResourceItems, exerciseResourcesSelector, searchExerciseResources, exerciseResourcesLoading } from "./collection-slice";
...
export const useExerciseResourcesSearchCollection = (exercise?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    const dispatch = useDispatch();
    const cmsStyles = useCmsStyles();
    const [initialised, setInitialised] = useState(false);
    const initialisedSubKey = useRef(exercise);
    ...
    const loadExerciseResources = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(exercise) dispatch(searchExerciseResources({ exercise: exercise, search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch, exercise]);
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchExerciseResources({
            exercise: exercise, 
    ...
    useEffect(() => {
        if (!loading && (!exerciseresources ||
        exerciseresources.length === 0) && 
        (!initialised || initialisedSubKey.current !== exercise)
          && exercise) {
            setInitialised(true);
            initialisedSubKey.current = exercise;
            loadExerciseResources({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        exerciseresources, 
        loadExerciseResources, 
        initialised, exercise
    ]);
    ...
    return {
        LoadMoreExerciseResourcesSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadExerciseResources({
                offset: exerciseresources?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }

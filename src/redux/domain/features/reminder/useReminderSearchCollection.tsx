...
import { clearReminderItems, remindersSelector, searchReminders, remindersLoading } from "./collection-slice";
...
export const useRemindersSearchCollection = (habit?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadReminders = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(habit) dispatch(searchReminders({ habit: habit, search: search, offset: offset || 0, limit: limit, filter: filter }));
    ...
    useEffect(() => {
        ...
        (!initialised || initialisedSubKey.current !== habit)
          && habit) {
            ...
            loadReminders({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        reminders, 
        loadReminders, 

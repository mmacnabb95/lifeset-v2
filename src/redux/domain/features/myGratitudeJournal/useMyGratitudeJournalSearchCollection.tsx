...
import { clearMyGratitudeJournalItems, myGratitudeJournalsSelector, searchMyGratitudeJournals, myGratitudeJournalsLoading } from "./collection-slice";
...
export const useMyGratitudeJournalsSearchCollection = (user?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const toggleFilter = (fieldName: string, filterTypeId?: number) => {
        ...
        dispatch(clearMyGratitudeJournalItems());
        reSearch();
    };
    ...
    const loadMyGratitudeJournals = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(user) dispatch(searchMyGratitudeJournals({ user: user, search: search, offset: offset || 0, limit: limit, filter: filter }));
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchMyGratitudeJournals({
            user: user, 
    ...
    useEffect(() => {
        if (!loading && (!mygratitudejournals ||
        mygratitudejournals.length === 0) && 
        (!initialised || initialisedSubKey.current !== user)
          && user) {
            setInitialised(true);
            initialisedSubKey.current = user;
            loadMyGratitudeJournals({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        mygratitudejournals, 
    ...
    return {
        LoadMoreMyGratitudeJournalsSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadMyGratitudeJournals({
                offset: mygratitudejournals?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }

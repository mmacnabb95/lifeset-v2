...
import { clearPublishedUserHabitPackItems, publishedUserHabitPacksSelector, searchPublishedUserHabitPacks, publishedUserHabitPacksLoading } from "./collection-slice";
...
export const usePublishedUserHabitPacksSearchCollection = ( initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadPublishedUserHabitPacks = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        dispatch(searchPublishedUserHabitPacks({ search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch]);
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchPublishedUserHabitPacks({
    ...
    useEffect(() => {
        if (!loading && (!publisheduserhabitpacks ||
        publisheduserhabitpacks.length === 0) && 
        (!initialised )
         ) {
            setInitialised(true);
            
            loadPublishedUserHabitPacks({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        publisheduserhabitpacks, 
    ...
    return {
        LoadMorePublishedUserHabitPacksSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadPublishedUserHabitPacks({
                offset: publisheduserhabitpacks?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }

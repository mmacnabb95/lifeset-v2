...
import { clearCompanyResourceItems, companyResourcesSelector, searchCompanyResources, companyResourcesLoading } from "./collection-slice";
...
export const useCompanyResourcesSearchCollection = (company?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadCompanyResources = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(company) dispatch(searchCompanyResources({ company: company, search: search, offset: offset || 0, limit: limit, filter: filter }));
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchCompanyResources({
            company: company, 
    ...
    return {
        LoadMoreCompanyResourcesSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadCompanyResources({
                offset: companyresources?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }

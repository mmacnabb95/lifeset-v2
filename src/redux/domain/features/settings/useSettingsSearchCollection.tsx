...
import { clearSettingsItems, settingssSelector, searchSettingss, settingssLoading } from "./collection-slice";
...
export const useSettingssSearchCollection = (id?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    const dispatch = useDispatch();
    const cmsStyles = useCmsStyles();
    const [initialised, setInitialised] = useState(false);
    const initialisedSubKey = useRef(id);
    ...
    const loadSettingss = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(id) dispatch(searchSettingss({ id: id, search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch, id]);
    ...
    useEffect(() => {
        if (!loading && (!settingss ||
        settingss.length === 0) && 
        (!initialised || initialisedSubKey.current !== id)
          && id) {
            setInitialised(true);
            initialisedSubKey.current = id;
            loadSettingss({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        settingss, 
        loadSettingss, 

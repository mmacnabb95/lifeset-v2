...
import { clearHabitItems, habitsSelector, searchHabits, habitsLoading } from "./collection-slice";
...
export const useHabitsSearchCollection = (user?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadHabits = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(user) dispatch(searchHabits({ user: user, search: search, offset: offset || 0, limit: limit, filter: filter }));
    ...
    useEffect(() => {
        ...
        (!initialised || initialisedSubKey.current !== user)
          && user) {
            ...
            loadHabits({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        habits, 
        loadHabits, 

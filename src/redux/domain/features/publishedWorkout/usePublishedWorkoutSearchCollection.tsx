...
import { clearPublishedWorkoutItems, publishedWorkoutsSelector, searchPublishedWorkouts, publishedWorkoutsLoading } from "./collection-slice";
...
export const usePublishedWorkoutsSearchCollection = ( initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadPublishedWorkouts = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        dispatch(searchPublishedWorkouts({ search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch]);
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchPublishedWorkouts({
    ...
    useEffect(() => {
        if (!loading && (!publishedworkouts ||
        publishedworkouts.length === 0) && 
        (!initialised )
         ) {
            setInitialised(true);
            
            loadPublishedWorkouts({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        publishedworkouts, 
    ...
    const Filters = ({
          ...
          return <ListFilter
                      ...
                      doSearch={doSearch}
                      reSearch={reSearch}
                      clearingThunk={clearPublishedWorkoutItems}
    ...
    return {
        LoadMorePublishedWorkoutsSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadPublishedWorkouts({
                offset: publishedworkouts?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }

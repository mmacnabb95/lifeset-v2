...
import { clearTodoListJournalItems, todoListJournalsSelector, searchTodoListJournals, todoListJournalsLoading } from "./collection-slice";
...
export const useTodoListJournalsSearchCollection = (user?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadTodoListJournals = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(user) dispatch(searchTodoListJournals({ user: user, search: search, offset: offset || 0, limit: limit, filter: filter }));
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchTodoListJournals({
            user: user, 
    ...
    useEffect(() => {
        if (!loading && (!todolistjournals ||
        todolistjournals.length === 0) && 
        (!initialised || initialisedSubKey.current !== user)
          && user) {
            setInitialised(true);
            initialisedSubKey.current = user;
            loadTodoListJournals({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        todolistjournals, 
    ...
    return {
        LoadMoreTodoListJournalsSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadTodoListJournals({
                offset: todolistjournals?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }

...
import { clearCategoryItems, categorysSelector, searchCategorys, categorysLoading } from "./collection-slice";
...
export const useCategorysSearchCollection = ( initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys, initialHardFilters?: HardFilter[]) => {
    ...
    const loadCategorys = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        dispatch(searchCategorys({ search: search, offset: offset || 0, limit: limit, filter: filter }));
    }, [search, dispatch]);
    ...
    useEffect(() => {
        if (!loading && (!categorys ||
        categorys.length === 0) && 
        (!initialised )
         ) {
            setInitialised(true);
            
            loadCategorys({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        categorys, 
        loadCategorys, 

...
import { clearSubscriptionViewItems, subscriptionViewsSelector, searchSubscriptionViews, subscriptionViewsLoading } from "./collection-slice";
...
export const useSubscriptionViewsSearchCollection = (user?: number,  initialLoadSize?: number, initialFilters?: InitialFilter[], filterKeys?: FilterKeys) => {
    ...
    const loadSubscriptionViews = useCallback(({ offset, limit, filter }: { offset?: number, limit: number, filter?: string }) => {
        setTextFilter(filter);
        setBasicParams({offset, limit, filter});
        if(user) dispatch(searchSubscriptionViews({ user: user, search: search, offset: offset || 0, limit: limit, filter: filter }));
    ...
    const reSearch = (_search?: Search) => {
        ...
        dispatch(searchSubscriptionViews({
            user: user, 
    ...
    useEffect(() => {
        if (!loading && (!subscriptionviews ||
        subscriptionviews.length === 0) && 
        (!initialised || initialisedSubKey.current !== user)
          && user) {
            setInitialised(true);
            initialisedSubKey.current = user;
            loadSubscriptionViews({ offset: 0, limit: initialLoadSize || 3, filter: textFilter });
        }
    }, [loading, 
        subscriptionviews, 
        loadSubscriptionViews, 
        initialised, user
    ]);
    ...
    return {
        LoadMoreSubscriptionViewsSearchButton: () => (
          <Button
            title="Load more"
            style={{ maxWidth: 400  }}
            ...
            onPress={() =>
              loadSubscriptionViews({
                offset: subscriptionviews?.length,
                limit: initialLoadSize || 3,
                filter: textFilter,
              })
            }
        ...
        searchResult: subscriptionviews,
        search,
        setSearch,
        doSearch,
        reSearch,

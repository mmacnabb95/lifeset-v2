...
import { createCompany, updateCompany, deleteCompany, companyErrorSelector, companySelector , clearCompanyError } from "src/redux/domain/features/company/collection-slice";
...
export const defaultTextValues:DefaultTextValues[] = [
  ...
  { propertyName : "BenefitsTitle", label : "Benefits title" , placeholder : "Benefits title"}, 
];
...
const CompanyForm = forwardRef(({
  ...
  const error = useSelector(companyErrorSelector);
  const companyItem = useSelector(companySelector(formItem?.Id));
  const [currentTextValues, setCurrentTextValues] = useState(defaultTextValues);

  
  const initialValues: Partial<Company> = {
      Name: companyItem?.Name || formItem?.Name || '',
      BenefitsTitle: companyItem?.BenefitsTitle || formItem?.BenefitsTitle || '',
  };
  ...
  const itemExists = () => {
    return companyItem || (formItem && formItem?.Id && formItem.Id > 0) ? true : false;
  };

  const getItemId = () => {
    if (companyItem?.Id) return { Id: companyItem?.Id };
    return {};
  }

  useImperativeHandle(ref, () => (formik));

  const formik = useFormik<Partial<Company>>({
    ...
    onSubmit: async (company) => {
      let response:any;
      if (itemExists()) {
        const updateObjectFull = { ...company, ...getItemId() } ;
        const updateObjectIgnored = _.omit(updateObjectFull, ["HasVideo", "HasImage", ]);
        response = await dispatch(updateCompany(updateObjectIgnored));

        if(afterUpdate) afterUpdate({...response, isValid});
      } else {
        response = await dispatch(createCompany(_.omit(formik.values, ["HasVideo", "HasImage"])));
  ...
  const handleDelete = async () => {
      if (!beforeDelete || await beforeDelete() === true) {
        const response: any = await dispatch(deleteCompany(companyItem?.Id as number));
  ...
  return (
    <View style={[formStyles.form, layoutStyles.parentForm, cmsStyles?.companyForm, style?.form]}>
      ...
      { buttonsTop && (
          <View style={[formStyles.buttons, style?.buttons]}>
            <Button
              ...
              testID={"submit-Company-form"}
              title={itemExists() ? updateButtonText ? updateButtonText : `Update Company` : saveButtonText ? saveButtonText : `Save Company`}
            ...
            {!hideDeleteButton && itemExists() && (
              <Button
                ...
                testID={"deleteCompany"}
                title={deleteButtonText ? deleteButtonText : `Delete Company`}
          ...
          <Input
            editable={viewOnly === true ? false : (readOnly && readOnly.indexOf('BenefitsTitle') !== -1 ? false : true)}
            multiline={fieldConfig['BenefitsTitle']?.multiline ? fieldConfig['BenefitsTitle'].multiline  : windowWidth > mobileBreak ? false : false}
            numberOfLines={fieldConfig['BenefitsTitle']?.numberOfLines ? fieldConfig['BenefitsTitle'].numberOfLines : 1}
            // scrollEnabled={false}
            inputLabelStyle={[
              style?.inputLabelStyle
            ]}
            ...
            testID="BenefitsTitle"
            placeholder={ returnCorrectTextValue("BenefitsTitle", "placeholder") }
            onChangeText={(text) => {
              formik.setFieldValue("BenefitsTitle", text, true)
            }}
            onBlur={formik.handleBlur("BenefitsTitle")}
            value={formik.values.BenefitsTitle?.toString()  || ""}
            onKeyPress={handleKeyDown}
            label={ returnCorrectTextValue("BenefitsTitle", "label") }
            errorMessage={formik.touched.BenefitsTitle && formik.errors.BenefitsTitle}
            errorPlace={'centerRight'}
            maxLength={45}
          />

        
          {fieldAppendees?.filter((f) => f.fieldName === 'BenefitsTitle').map((fa) => {
            return fa.el;
          })}

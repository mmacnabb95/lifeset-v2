...
import { createUserHabitPack, updateUserHabitPack, deleteUserHabitPack, userHabitPackErrorSelector, userHabitPackSelector , clearUserHabitPackError } from "src/redux/domain/features/userHabitPack/collection-slice";
...
const UserHabitPackForm = forwardRef(({
  customTextValues,
  afterCreate,
  ...
  formItem?: Partial<Userhabitpack>,
  ...
  handleValuesChange?: ({
    ...
    values: Partial<Userhabitpack>,
    errors: FormikErrors<Userhabitpack>
  }) => void,
  ...
  const error = useSelector(userHabitPackErrorSelector);
  const userHabitPackItem = useSelector(userHabitPackSelector(formItem?.Id));
  const [currentTextValues, setCurrentTextValues] = useState(defaultTextValues);

  
  const initialValues: Partial<Userhabitpack> = {
      User: userHabitPackItem?.User || formItem?.User || undefined,
      Name: userHabitPackItem?.Name || formItem?.Name || '',
      UserHabitPackStatus: userHabitPackItem?.UserHabitPackStatus || formItem?.UserHabitPackStatus || undefined,
  };
  ...
  const itemExists = () => {
    return userHabitPackItem || (formItem && formItem?.Id && formItem.Id > 0) ? true : false;
  };

  const getItemId = () => {
    if (userHabitPackItem?.Id) return { Id: userHabitPackItem?.Id };
    return {};
  }

  useImperativeHandle(ref, () => (formik));

  const formik = useFormik<Partial<Userhabitpack>>({
    initialValues: { ...initialValues, ...formItem },
    validationSchema: Yup.object({
        Name: Yup.string().required(textValues?.requiredText || 'Required'),
        ...injectedValidationSchema,
    }),
    onSubmit: async (userHabitPack) => {
      let response:any;
      if (itemExists()) {
        const updateObjectFull = { ...userHabitPack, ...getItemId() } ;
        const updateObjectIgnored = _.omit(updateObjectFull, ["HasVideo", "HasImage", ]);
        response = await dispatch(updateUserHabitPack(updateObjectIgnored));

        if(afterUpdate) afterUpdate({...response, isValid});
      } else {
        response = await dispatch(createUserHabitPack(_.omit(formik.values, ["HasVideo", "HasImage"])));
  ...
  const handleDelete = async () => {
      if (!beforeDelete || await beforeDelete() === true) {
        const response: any = await dispatch(deleteUserHabitPack(userHabitPackItem?.Id as number));
  ...
  useFocusEffect(
    React.useCallback(() => {
      dispatch(clearUserHabitPackError());
    }, [dispatch]),
  );

  return (
    <View style={[formStyles.form, layoutStyles.parentForm, cmsStyles?.userHabitPackForm, style?.form]}>
      ...
      { buttonsTop && (
          <View style={[formStyles.buttons, style?.buttons]}>
            <Button
              ...
              testID={"submit-UserHabitPack-form"}
              title={itemExists() ? updateButtonText ? updateButtonText : `Update User Habit Pack` : saveButtonText ? saveButtonText : `Save User Habit Pack`}
            ...
            {!hideDeleteButton && itemExists() && (
              <Button
                ...
                testID={"deleteUserHabitPack"}
                title={deleteButtonText ? deleteButtonText : `Delete User Habit Pack`}
...
export default UserHabitPackForm;

...
import { createHabitPackHabit, updateHabitPackHabit, deleteHabitPackHabit, habitPackHabitErrorSelector, habitPackHabitSelector , clearHabitPackHabitError } from "src/redux/domain/features/habitPackHabit/collection-slice";
...
export const defaultTextValues:DefaultTextValues[] = [
  ...
  { propertyName : "Category", label : "Category" , placeholder : "Category"}, 
];

const HabitPackHabitForm = forwardRef(({
  customTextValues,
  afterCreate,
  ...
  formItem?: Partial<Habitpackhabit>,
  ...
  handleValuesChange?: ({
    ...
    values: Partial<Habitpackhabit>,
    errors: FormikErrors<Habitpackhabit>
  }) => void,
  ...
  const error = useSelector(habitPackHabitErrorSelector);
  const habitPackHabitItem = useSelector(habitPackHabitSelector(formItem?.Id));
  const [currentTextValues, setCurrentTextValues] = useState(defaultTextValues);

  
  const initialValues: Partial<Habitpackhabit> = {
      HabitPack: habitPackHabitItem?.HabitPack || formItem?.HabitPack || undefined,
      Name: habitPackHabitItem?.Name || formItem?.Name || '',
      Description: habitPackHabitItem?.Description || formItem?.Description || '',
      Category: habitPackHabitItem?.Category || formItem?.Category || undefined,
  };
  ...
  const itemExists = () => {
    return habitPackHabitItem || (formItem && formItem?.Id && formItem.Id > 0) ? true : false;
  };

  const getItemId = () => {
    if (habitPackHabitItem?.Id) return { Id: habitPackHabitItem?.Id };
    return {};
  }

  useImperativeHandle(ref, () => (formik));

  const formik = useFormik<Partial<Habitpackhabit>>({
    ...
    onSubmit: async (habitPackHabit) => {
      let response:any;
      if (itemExists()) {
        const updateObjectFull = { ...habitPackHabit, ...getItemId() } ;
        const updateObjectIgnored = _.omit(updateObjectFull, ["HasVideo", "HasImage", ]);
        response = await dispatch(updateHabitPackHabit(updateObjectIgnored));

        if(afterUpdate) afterUpdate({...response, isValid});
      } else {
        response = await dispatch(createHabitPackHabit(_.omit(formik.values, ["HasVideo", "HasImage"])));
  ...
  const handleDelete = async () => {
      if (!beforeDelete || await beforeDelete() === true) {
        const response: any = await dispatch(deleteHabitPackHabit(habitPackHabitItem?.Id as number));
  ...
  useFocusEffect(
    React.useCallback(() => {
      dispatch(clearHabitPackHabitError());
    }, [dispatch]),
  );

  return (
    <View style={[formStyles.form, cmsStyles?.habitPackHabitForm, style?.form]}>
      ...
      { buttonsTop && (
          <View style={[formStyles.buttons, style?.buttons]}>
            <Button
              ...
              testID={"submit-HabitPackHabit-form"}
              title={itemExists() ? updateButtonText ? updateButtonText : `Update Habit Pack Habit` : saveButtonText ? saveButtonText : `Save Habit Pack Habit`}
            ...
            {!hideDeleteButton && itemExists() && (
              <Button
                ...
                testID={"deleteHabitPackHabit"}
                title={deleteButtonText ? deleteButtonText : `Delete Habit Pack Habit`}
...
export default HabitPackHabitForm;

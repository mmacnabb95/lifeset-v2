...
import { clearDaysPerWeekItems } from "src/redux/domain/features/daysPerWeek/collection-slice";
// import { deleteDaysPerWeek, updateDaysPerWeek } from "src/redux/domain/features/daysPerWeek/item-slice";
import { Daysperweek } from "../../../../../../types/domain/flat-types";
import { useDaysPerWeekCollection } from "src/redux/domain/features/daysPerWeek/useDaysPerWeekCollection";
import { initialLoadSize, rootLanguage } from "src/utils";
import { useDaysPerWeeksSearchCollection } from "src/redux/domain/features/daysPerWeek/useDaysPerWeekSearchCollection";
import { daysPerWeeksLoading } from "src/redux/domain/features/daysPerWeek/collection-slice";
...
const DaysPerWeekViewListScreen = ({
  navigation,
  route,
  showTitle = true,
  // showPreamble = true,
  ...
  const viewListConfig = useViewListConfig("DaysPerWeek", route);
  ...
  const {
    ...
    loadDaysPerWeeks,
    basicParams,
    searchResult: daysPerWeeks,
    Filters,
  } = useDaysPerWeeksSearchCollection(initialLoadSize);
  ...
  const loading = useSelector(daysPerWeeksLoading);
      ...
      >
        <ScrollView
          ...
          contentContainerStyle={[
            layoutStyles.body,
            cmsStyles?.daysPerWeekViewListContentContainerStyle,
          ]}
        ...
        >
          <Header
            title={
              showTitle
                ? text("daysPerWeekViewList.title")[0] || "DaysPerWeeks"
                : ""
            }
            preamble={text("daysPerWeekViewList.preamble")[0] || ""}
            ...
            style={[
              ...
              cmsStyles?.daysPerWeekViewListHeaderStyle,
            ]}
            titleStyle={cmsStyles?.daysPerWeekViewListTitleStyle}
          />
          ...
          <ListBody
            ...
            loadMore={loadMore}
            doLoad={loadDaysPerWeeks}
            destination={destination}
            orderBy={["Order"]}
            paramKey="daysPerWeekId"
...
export default DaysPerWeekViewListScreen;

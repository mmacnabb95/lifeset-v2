...
import BenefitForm, { defaultTextValues } from "src/components/domain/benefit/benefit.form";
...
import { clearBenefitItems, createBenefit, updateBenefit } from "src/redux/domain/features/benefit/collection-slice";
import {
  getBenefit,
  benefitLoading,
  benefitSelector,
...
const BenefitEditScreen = React.forwardRef(({
  navigation,
  route,
  showHeader = true,
  hideButtons = false,
  ...
  const { viewOnly, benefitId, companyId,  } = route.params;
  const benefit: Benefit = useSelector(
    benefitSelector(benefitId === "new" ? -1 : benefitId),
  );
  const loading = useSelector(benefitLoading);
  const [wasNew, setWasNew] = useState(benefitId === "new");
  ...
  const formConfig = getConfig("Benefit", benefit, formRef, navigation, route);

  useFocusEffect(
    React.useCallback(() => {
      if (benefitId !== "new" && wasNew === false) {
         dispatch(getBenefit(benefitId));
      }
    }, [dispatch, benefitId, wasNew]),
  );
  ...
  getAppendees("Benefit", fieldAppendees, benefit, formRef, navigation, route);
  ...
  return (
    <>
      <WebFadeIn background={true}>
        <View style={[[layoutStyles.page, cmsStyles?.benefitEditPage]]}>
          {showHeader && <Header navigation={navigation}/>}
          <Body contentStyle={[cmsStyles?.benefitEditPageBody]}>
            <>
              
              {(benefitId === "new" || (benefit !== undefined && benefit !== "")) && (
                <BenefitForm
                  ref={formRef}
                  key={benefitId}
                  fieldAppendees={fieldAppendees}
                  formItem={benefit || formConfig?.default || {Company: companyId, }}
                  viewOnly={viewOnly}
                  ...
                  afterCreate={(response: any) => {
                    if (!handleError(response)) {
                      ...
                      navigation.setParams({benefitId: response.payload.Id});
                      setAlertTitle(t("benefit.onCreated") || "Benefit created");
                      setAlertIcon("check");
                  ...
                  afterUpdate={(response: any) => {
                    if (!handleError(response)) {
                      setAlertTitle(t("benefit.onUpdated") || "Benefit updated");
                      setAlertIcon("check");
                  ...
                  afterDelete={(response) => {
                    if (!handleError(response)) {
                      setAlertTitle(t("benefit.onDeleted") || "Benefit deleted");
                      setAlertIcon("check");
                      setIsAlertOpen(true);
              ...
              {(loading && (benefit === undefined || benefit === "")) &&
                benefitId !== "new" && (
                  <View>
      ...
      <Modal
        ...
        title={t("benefit.deleteWarningTitle") || "Delete Benefit"}
        text={t("benefit.deleteWarningText") || "Are you sure you want to delete this Benefit?"}
        acceptButton={(
          <Button
            title="Delete"
            onPress={() => {
...
export default BenefitEditScreen;

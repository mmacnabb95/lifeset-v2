...
import { clearFitnessGoalItems } from "src/redux/domain/features/fitnessGoal/collection-slice";
// import { deleteFitnessGoal, updateFitnessGoal } from "src/redux/domain/features/fitnessGoal/item-slice";
import { Fitnessgoal } from "../../../../../../types/domain/flat-types";
import { useFitnessGoalCollection } from "src/redux/domain/features/fitnessGoal/useFitnessGoalCollection";
import { initialLoadSize, rootLanguage } from "src/utils";
import { useFitnessGoalsSearchCollection } from "src/redux/domain/features/fitnessGoal/useFitnessGoalSearchCollection";
import { fitnessGoalsLoading } from "src/redux/domain/features/fitnessGoal/collection-slice";
...
const FitnessGoalViewListScreen = ({
  navigation,
  route,
  showTitle = true,
  // showPreamble = true,
  ...
  const viewListConfig = useViewListConfig("FitnessGoal", route);
  ...
  const {
    ...
    loadFitnessGoals,
    basicParams,
    searchResult: fitnessGoals,
    Filters,
  } = useFitnessGoalsSearchCollection(initialLoadSize);
  ...
  const loading = useSelector(fitnessGoalsLoading);
      ...
      >
        <ScrollView
          ...
          contentContainerStyle={[
            layoutStyles.body,
            cmsStyles?.fitnessGoalViewListContentContainerStyle,
          ]}
        ...
        >
          <Header
            title={
              showTitle
                ? text("fitnessGoalViewList.title")[0] || "FitnessGoals"
                : ""
            }
            preamble={text("fitnessGoalViewList.preamble")[0] || ""}
            ...
            style={[
              ...
              cmsStyles?.fitnessGoalViewListHeaderStyle,
            ]}
            titleStyle={cmsStyles?.fitnessGoalViewListTitleStyle}
          />
          ...
          <ListBody
            ...
            loadMore={loadMore}
            doLoad={loadFitnessGoals}
            destination={destination}
            orderBy={["Order"]}
            paramKey="fitnessGoalId"
...
export default FitnessGoalViewListScreen;

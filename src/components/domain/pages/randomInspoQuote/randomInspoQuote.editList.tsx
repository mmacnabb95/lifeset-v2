...
import { clearRandomInspoQuoteItems } from "src/redux/domain/features/randomInspoQuote/collection-slice";
// import { deleteRandomInspoQuote, updateRandomInspoQuote } from "src/redux/domain/features/randomInspoQuote/item-slice";
...
import { useRandomInspoQuotesSearchCollection } from "src/redux/domain/features/randomInspoQuote/useRandomInspoQuoteSearchCollection";
import { randomInspoQuotesLoading } from "src/redux/domain/features/randomInspoQuote/collection-slice";
...
import TextSearch from "src/redux/domain/features/randomInspoQuote/textSearch";
import { useUserInfo } from "src/redux/features/userInfo/useUserInfo";

const RandomInspoQuoteEditListScreen = ({
  navigation,
  route,
  showTitle = true,
  // showPreamble = true,
  ...
  scrollEnabled = true,
  destination = "RandomInspoQuoteEdit",
  ...
  const editListConfig = useEditListConfig("RandomInspoQuote", route);
  ...
  const {
    loadMore,
    loadRandomInspoQuotes,
    basicParams,
    searchResult: randomInspoQuotes,
    Filters
  } = useRandomInspoQuotesSearchCollection(initialLoadSize)
  ...
  const loading = useSelector(randomInspoQuotesLoading);
  ...
  return (
    <WebFadeIn off={!fadeIn} style={style} background={true}>
      <View style={[layoutStyles.page, layoutStyles.paddingMob20, layoutStyles.parentEditList, cmsStyles?.randomInspoQuoteEditList, style]}>
        <ScrollView
          scrollEnabled={scrollEnabled}
          nestedScrollEnabled={true}
          style={[layoutStyles.scrollViewContainer]}
          contentContainerStyle={[layoutStyles.body,cmsStyles?.randomInspoQuoteEditListContentContainerStyle ]}
          showsVerticalScrollIndicator={false}
          onScroll={scrollCallback}
          scrollEventThrottle={1000}
        >
        <Header
          title={showTitle ? text("randomInspoQuoteEditList.title")[0] || "Random inspo quotes" : ""}
          preamble={text("randomInspoQuoteEditList.preamble")[0] || ""}
          navigation={navigation}
          addNewDestination={route.params?.viewOnly || newPosition === "bottom" ? undefined :"RandomInspoQuoteEdit"}
          addNewParams={{ randomInspoQuoteId: "new" }}
          
          hideGoBack={width < commonConstants.mobileBreak || hideGoBack}
          newEnabled={newEnabled}
          style={[style, layoutStyles.editListHeaderStyle, cmsStyles?.randomInspoQuoteEditListHeaderStyle]}
          titleStyle={[layoutStyles.parentListPageTitle, cmsStyles?.randomInspoQuoteEditListTitleStyle]}
          underHeaderComponents={
            <View style={layoutStyles.searchSection}>
            <TextSearch
              navigation={navigation}
              loadRandomInspoQuotes={loadRandomInspoQuotes}
        ...
        <ListBody
          navigation={navigation}
          route={route}
          listItems={randomInspoQuotes}
          loading={loading}
          style={style}
          contentStyle={contentStyle}
          basicParams={basicParams}
          loadMore={loadMore}
          doLoad={loadRandomInspoQuotes}
          destination={destination}
          orderBy={['Order', ]}
          paramKey="randomInspoQuoteId"
        ...
        {!route.params?.viewOnly && newPosition === "bottom" && (
          <AddNew
            destinationScreen={route.params?.viewOnly ? "" : "RandomInspoQuoteEdit"}
            params={{ randomInspoQuoteId: "new" }}
...
export default RandomInspoQuoteEditListScreen;

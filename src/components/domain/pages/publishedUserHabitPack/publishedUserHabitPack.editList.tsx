...
// import { deletePublishedUserHabitPack, updatePublishedUserHabitPack } from "src/redux/domain/features/publishedUserHabitPack/item-slice";
import { Publisheduserhabitpack } from "../../../../../../types/domain/flat-types";
import { usePublishedUserHabitPackCollection } from "src/redux/domain/features/publishedUserHabitPack/usePublishedUserHabitPackCollection";
...
import TextSearch from "src/redux/domain/features/publishedUserHabitPack/textSearch";
import { useUserInfo } from "src/redux/features/userInfo/useUserInfo";

const PublishedUserHabitPackEditListScreen = ({
  navigation,
  route,
  showTitle = true,
  // showPreamble = true,
  ...
  scrollEnabled = true,
  destination = "PublishedUserHabitPackEdit",
  ...
  const editListConfig = useEditListConfig("PublishedUserHabitPack", route);
  ...
  const {
    loadMore,
    loadPublishedUserHabitPacks,
    basicParams,
    searchResult: publishedUserHabitPacks,
    Filters
  ...
  return (
    <WebFadeIn off={!fadeIn} style={style} background={true}>
      <View style={[layoutStyles.page, layoutStyles.paddingMob20, layoutStyles.parentEditList, cmsStyles?.publishedUserHabitPackEditList, style]}>
        <ScrollView
          scrollEnabled={scrollEnabled}
          nestedScrollEnabled={true}
          style={[layoutStyles.scrollViewContainer]}
          contentContainerStyle={[layoutStyles.body,cmsStyles?.publishedUserHabitPackEditListContentContainerStyle ]}
          showsVerticalScrollIndicator={false}
          onScroll={scrollCallback}
          scrollEventThrottle={1000}
        >
        <Header
          title={showTitle ? text("publishedUserHabitPackEditList.title")[0] || "Published user habit packs" : ""}
          preamble={text("publishedUserHabitPackEditList.preamble")[0] || ""}
          navigation={navigation}
          addNewDestination={route.params?.viewOnly || newPosition === "bottom" ? undefined :"PublishedUserHabitPackEdit"}
          addNewParams={{ publishedUserHabitPackId: "new" }}
          
          hideGoBack={width < commonConstants.mobileBreak || hideGoBack}
          newEnabled={newEnabled}
          style={[style, layoutStyles.editListHeaderStyle, cmsStyles?.publishedUserHabitPackEditListHeaderStyle]}
          titleStyle={[layoutStyles.parentListPageTitle, cmsStyles?.publishedUserHabitPackEditListTitleStyle]}
          underHeaderComponents={
            <View style={layoutStyles.searchSection}>
            <TextSearch
              navigation={navigation}
              loadPublishedUserHabitPacks={loadPublishedUserHabitPacks}
        ...
        <ListBody
          navigation={navigation}
          route={route}
          listItems={publishedUserHabitPacks}
          loading={loading}
          style={style}
          contentStyle={contentStyle}
          basicParams={basicParams}
          loadMore={loadMore}
          doLoad={loadPublishedUserHabitPacks}
        ...
        {!route.params?.viewOnly && newPosition === "bottom" && (
          <AddNew
            destinationScreen={route.params?.viewOnly ? "" : "PublishedUserHabitPackEdit"}
            params={{ publishedUserHabitPackId: "new" }}
...
export default PublishedUserHabitPackEditListScreen;

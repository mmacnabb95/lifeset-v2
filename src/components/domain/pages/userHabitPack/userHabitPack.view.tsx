...
const UserHabitPackViewScreen = ({
  ...
  const userHabitPack: Userhabitpack = useSelector(
    userHabitPackSelector(userHabitPackId),
  );
  ...
  useFocusEffect(
    React.useCallback(() => {
      if (userHabitPackId !== "new") {
        dispatch(getUserHabitPack(userHabitPackId));
      }
    }, [dispatch, userHabitPackId]),
  );
  ...
  return (
    <WebFadeIn>
      <View style={[videoFullScreen ? layoutStyles.fullScreenPage : [layoutStyles.page, cmsStyles?.userHabitPack]]}>
        ...
        <Body  onScrollCallback={scrollCallback}>
          <View style={[formStyles.form, {flexGrow: 1}]}>
            {loading && !userHabitPack && <Text>{text("common.loading")}...</Text>}
            
            <View style={[videoFullScreen ? {} : [layoutStyles.itemPage]]}>
              <Slot
                index={'first'}
                componentName={"UserHabitPack"}
                navigation={navigation}
                route={route}
                source={userHabitPack}
              />
              <Typography type={TypographyTypes.H1} style={[cmsStyles.title, layoutStyles.title]} text={userHabitPack?.Title || userHabitPack?.Name} />
              
              { userHabitPack?.Introduction && (
                <Typography type={TypographyTypes.Body1} style={[cmsStyles.intro, layoutStyles.intro]} text={userHabitPack?.Introduction} />
              )}
              
              <Typography type={TypographyTypes.Body1} style={[cmsStyles.article, layoutStyles.article]} text={userHabitPack?.Article || userHabitPack?.Description || userHabitPack?.Text} />
              <Slot
                index={'last'}
                componentName={"UserHabitPack"}
                navigation={navigation}
...
export default UserHabitPackViewScreen;

...
import CompanyUserForm, { defaultTextValues } from "src/components/domain/companyUser/companyUser.form";
...
import { clearCompanyUserItems, createCompanyUser, updateCompanyUser } from "src/redux/domain/features/companyUser/collection-slice";
import {
  getCompanyUser,
  companyUserLoading,
  companyUserSelector,
...
const CompanyUserEditScreen = React.forwardRef(({
  navigation,
  route,
  showHeader = true,
  hideButtons = false,
  ...
  const { viewOnly, companyUserId, companyId,  } = route.params;
  const companyUser: Companyuser = useSelector(
    companyUserSelector(companyUserId === "new" ? -1 : companyUserId),
  );
  const loading = useSelector(companyUserLoading);
  const [wasNew, setWasNew] = useState(companyUserId === "new");
  ...
  const formConfig = getConfig("CompanyUser", companyUser, formRef, navigation, route);

  useFocusEffect(
    React.useCallback(() => {
      if (companyUserId !== "new" && wasNew === false) {
         dispatch(getCompanyUser(companyUserId));
      }
    }, [dispatch, companyUserId, wasNew]),
  );
  ...
  getAppendees("CompanyUser", fieldAppendees, companyUser, formRef, navigation, route);
  ...
  return (
    <>
      <WebFadeIn background={true}>
        <View style={[[layoutStyles.page, cmsStyles?.companyUserEditPage]]}>
          {showHeader && <Header navigation={navigation}/>}
          <Body contentStyle={[cmsStyles?.companyUserEditPageBody]}>
            <>
              
              {(companyUserId === "new" || (companyUser !== undefined && companyUser !== "")) && (
                <CompanyUserForm
                  ref={formRef}
                  key={companyUserId}
                  fieldAppendees={fieldAppendees}
                  formItem={companyUser || formConfig?.default || {Company: companyId, }}
                  viewOnly={viewOnly}
                  ...
                  afterCreate={(response: any) => {
                    if (!handleError(response)) {
                      ...
                      navigation.setParams({companyUserId: response.payload.Id});
                      setAlertTitle(t("companyUser.onCreated") || "Company User created");
                      setAlertIcon("check");
                  ...
                  afterUpdate={(response: any) => {
                    if (!handleError(response)) {
                      setAlertTitle(t("companyUser.onUpdated") || "Company User updated");
                      setAlertIcon("check");
                  ...
                  afterDelete={(response) => {
                    if (!handleError(response)) {
                      setAlertTitle(t("companyUser.onDeleted") || "Company User deleted");
                      setAlertIcon("check");
                      setIsAlertOpen(true);
              ...
              {(loading && (companyUser === undefined || companyUser === "")) &&
                companyUserId !== "new" && (
                  <View>
      ...
      <Modal
        ...
        title={t("companyUser.deleteWarningTitle") || "Delete Company User"}
        text={t("companyUser.deleteWarningText") || "Are you sure you want to delete this Company User?"}
        acceptButton={(
          <Button
            title="Delete"
            onPress={() => {

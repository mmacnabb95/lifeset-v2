...
const CompanyUserEditListScreen = ({
  ...
  const editListConfig = useEditListConfig("CompanyUser", route);
  ...
  // const { results: companyUsers, Refresh, initialised } = useCompanyUserCollection(companyId === "new" ? undefined : companyId, 100);
  ...
  const {
    ...
    Filters,
  } = useCompanyUsersSearchCollection(companyId === "new" ? undefined : companyId, initialLoadSize, undefined, );
  ...
  return (
    <WebFadeIn off={!fadeIn} style={style} background={true}>
      <View style={[layoutStyles.page, layoutStyles.paddingMob20, layoutStyles.childSubList, cmsStyles?.companyUserEditList, style]}>
        <ScrollView
          scrollEnabled={scrollEnabled}
          nestedScrollEnabled={true}
          style={[layoutStyles.scrollViewContainer]}
          contentContainerStyle={[layoutStyles.body, layoutStyles.childSubListScrollContent, cmsStyles?.companyUserEditListContentContainerStyle ]}
          showsVerticalScrollIndicator={false}
          onScroll={scrollCallback}
          scrollEventThrottle={1000}
        >
        <Header
          title={showTitle ? text("companyUserEditList.title")[0] || "Company users" : ""}
          preamble={text("companyUserEditList.preamble")[0] || ""}
          navigation={navigation}
          addNewDestination={route.params?.viewOnly || newPosition === "bottom" ? undefined :"CompanyUserEdit"}
          addNewParams={{ companyUserId: "new", companyId }}
          bulkUpload={editListConfig?.bulkUpload}
          hideGoBack={width < commonConstants.mobileBreak || hideGoBack}
          newEnabled={newEnabled}
          style={[style, cmsStyles?.companyUserEditListHeaderStyle]}
          titleStyle={[layoutStyles.childSubListPageTitle, cmsStyles?.companyUserEditListTitleStyle]}
        ...
        {!route.params?.viewOnly && newPosition === "bottom" && (
          <AddNew
            destinationScreen={route.params?.viewOnly ? "" : "CompanyUserEdit"}
            params={{ companyUserId: "new" }}

...
import { clearUserHabitPackStreakLeaderboardItems } from "src/redux/domain/features/userHabitPackStreakLeaderboard/collection-slice";
// import { deleteUserHabitPackStreakLeaderboard, updateUserHabitPackStreakLeaderboard } from "src/redux/domain/features/userHabitPackStreakLeaderboard/item-slice";
import { Userhabitpackstreakleaderboard } from "../../../../../../types/domain/flat-types";
import { useUserHabitPackStreakLeaderboardCollection } from "src/redux/domain/features/userHabitPackStreakLeaderboard/useUserHabitPackStreakLeaderboardCollection";
...
import { userHabitPackStreakLeaderboardsLoading } from "src/redux/domain/features/userHabitPackStreakLeaderboard/collection-slice";
...
import TextSearch from "src/redux/domain/features/userHabitPackStreakLeaderboard/textSearch";
import { useUserInfo } from "src/redux/features/userInfo/useUserInfo";

const UserHabitPackStreakLeaderboardViewListScreen = ({
  navigation,
  route,
  showTitle = true,
  // showPreamble = true,
  ...
  scrollEnabled = true,
  destination = "UserHabitPackStreakLeaderboardView",
  ...
  const viewListConfig = useViewListConfig("UserHabitPackStreakLeaderboard", route);
  ...
  const {
    ...
    basicParams,
    searchResult: userHabitPackStreakLeaderboards,
    Filters
  } = useUserHabitPackStreakLeaderboardsSearchCollection(initialLoadSize)
  ...
  const loading = useSelector(userHabitPackStreakLeaderboardsLoading);
  ...
  return (
    <WebFadeIn off={!fadeIn} style={style} background={true}>
      <View style={[layoutStyles.page, layoutStyles.paddingMob20, layoutStyles.parentViewList, cmsStyles?.userHabitPackStreakLeaderboardViewList, style]}>
        <ScrollView
          scrollEnabled={scrollEnabled}
          nestedScrollEnabled={true}
          style={[layoutStyles.scrollViewContainer]}
          contentContainerStyle={[layoutStyles.body, cmsStyles?.userHabitPackStreakLeaderboardViewListContentContainerStyle ]}
          showsVerticalScrollIndicator={false}
          onScroll={scrollCallback}
          scrollEventThrottle={1000}
        >
        <Header
          title={showTitle ? text("userHabitPackStreakLeaderboardViewList.title")[0] || "UserHabitPackStreakLeaderboards" : ""}
          preamble={text("userHabitPackStreakLeaderboardViewList.preamble")[0] || ""}
          navigation={navigation}
          addNewDestination={undefined}
          
          hideGoBack={width < commonConstants.mobileBreak || hideGoBack}
          newEnabled={false}
          style={[style, layoutStyles.viewListHeaderStyle, cmsStyles?.userHabitPackStreakLeaderboardViewListHeaderStyle]}
          titleStyle={cmsStyles?.userHabitPackStreakLeaderboardViewListTitleStyle}
          underHeaderComponents={
            <View style={layoutStyles.searchSection}>
            <TextSearch
              navigation={navigation}
              loadUserHabitPackStreakLeaderboards={loadUserHabitPackStreakLeaderboards}
        ...
        <ListBody
          navigation={navigation}
          route={route}
          listItems={userHabitPackStreakLeaderboards}
          mode="view"
          loading={loading}
          style={style}
          ...
          orderBy={['Order',  'MaxStreakThisMonth desc']}
          paramKey="userHabitPackStreakLeaderboardId"
...
export default UserHabitPackStreakLeaderboardViewListScreen;

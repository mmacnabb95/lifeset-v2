...
import WorkoutDayExerciseEdit from "src/components/domain/pages/workoutDayExercise/workoutDayExercise.edit"
...
import { useWorkoutDayExerciseInlineEditListDrawerHandler } from "src/components/common/config/hooks/useWorkoutDayExerciseInlineEditListDrawerHandler";
...
const WorkoutDayExerciseEditListScreen = ({
  ...
  const {onOpen, onClose} = useWorkoutDayExerciseInlineEditListDrawerHandler(setDrawerOpen, setDrawerItem);
  ...
  const { workoutDayId,  } = route.params; //lessonId === "new" ? undefined : lessonId, 1
  const {
    loadMore,
    ...
    search,
    reSearch,
  } = useWorkoutDayExercisesSearchCollection(workoutDayId === "new" ? undefined : workoutDayId, 100, undefined, );
  ...
  return (
    <Drawer open={drawerOpen}  handleClose={handleClose} leftItem={
      <WebFadeIn off={!fadeIn} style={style}>
        <View style={[layoutStyles.page, layoutStyles.paddingMob20, cmsStyles?.workoutDayExerciseEditList, style]}>
              <ScrollView
                scrollEnabled={true}
              ...
              >
          <Header
            title={showTitle ? text("workoutDayExerciseEditList.title")[0] || "WorkoutDayExercises" : ""}
            // preamble={showPreamble ? text("workoutDayExerciseEditList.preamble")[0] || "All workoutdayexercises" : ""}
            navigation={navigation}
            addNewDestination="WorkoutDayExerciseEdit"
        ...
        <>
          {drawerItem === 0 && <WorkoutDayExerciseEdit navigation={navigation} route={route} showHeader={false}/>}

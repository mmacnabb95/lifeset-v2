...
import WorkoutForm, { defaultTextValues } from "src/components/domain/workout/workout.form";
...
import { clearWorkoutItems, createWorkout, updateWorkout } from "src/redux/domain/features/workout/collection-slice";
import {
  getWorkout,
  workoutLoading,
  workoutSelector,
...
import WorkoutDayEditListScreen from "../workoutDay/workoutDay.editList";
...
const WorkoutEditScreen = React.forwardRef(({
  navigation,
  route,
  showHeader = true,
  hideButtons = false,
  ...
  const { viewOnly, workoutId,  } = route.params;
  const workout: Workout = useSelector(
    workoutSelector(workoutId === "new" ? -1 : workoutId),
  );
  const loading = useSelector(workoutLoading);
  const [wasNew, setWasNew] = useState(workoutId === "new");
  ...
  const formConfig = getConfig("Workout", workout, formRef, navigation, route);

  useFocusEffect(
    React.useCallback(() => {
      if (workoutId !== "new" && wasNew === false) {
         dispatch(getWorkout(workoutId));
      }
    }, [dispatch, workoutId, wasNew]),
  );
  ...
  getAppendees("Workout", fieldAppendees, workout, formRef, navigation, route);
  ...
  fieldAppendees.push({
    ...
    el: (
      <>
        ...
        <WorkoutDayEditListScreen
          key={`sublist_${workoutId}_0`}
          navigation={navigation}
          style={layoutStyles.subList}
          hideGoBack={true}
          showTitle={true}
          route={route}
          newEnabled={workoutId !== "new" && !formConfig?.viewOnly}
          fadeIn={false}
  ...
  return (
    <>
      <WebFadeIn background={true}>
        <View style={[[layoutStyles.page, layoutStyles.parentEditPage, cmsStyles?.workoutEditPage]]}>
          {showHeader && <Header navigation={navigation}/>}
          <Body contentStyle={[cmsStyles?.workoutEditPageBody]}>
            <>
              
              {(workoutId === "new" || (workout !== undefined && workout !== "")) && (
                <WorkoutForm
                  ref={formRef}
                  key={workoutId}
                  fieldAppendees={fieldAppendees}
                  formItem={workout || formConfig?.default}
                  viewOnly={viewOnly}
                  ...
                  afterCreate={(response: any) => {
                    if (!handleError(response)) {
                      ...
                      navigation.setParams({workoutId: response.payload.Id});
                      setAlertTitle(t("workout.onCreated") || "Workout created");
                      setAlertIcon("check");
                  ...
                  afterUpdate={(response: any) => {
                    if (!handleError(response)) {
                      setAlertTitle(t("workout.onUpdated") || "Workout updated");
                      setAlertIcon("check");
                  ...
                  afterDelete={(response) => {
                    if (!handleError(response)) {
                      setAlertTitle(t("workout.onDeleted") || "Workout deleted");
                      setAlertIcon("check");
                      setIsAlertOpen(true);
              ...
              {(loading && (workout === undefined || workout === "")) &&
                workoutId !== "new" && (
                  <View>
      ...
      <Modal
        ...
        title={t("workout.deleteWarningTitle") || "Delete Workout"}
        text={t("workout.deleteWarningText") || "Are you sure you want to delete this Workout?"}
        acceptButton={(
          <Button
            title="Delete"
            onPress={() => {

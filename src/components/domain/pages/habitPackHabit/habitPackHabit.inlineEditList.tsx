...
import HabitPackHabitEdit from "src/components/domain/pages/habitPackHabit/habitPackHabit.edit"
...
import { useHabitPackHabitInlineEditListDrawerHandler } from "src/components/common/config/hooks/useHabitPackHabitInlineEditListDrawerHandler";
...
const HabitPackHabitEditListScreen = ({
  ...
  const {onOpen, onClose} = useHabitPackHabitInlineEditListDrawerHandler(setDrawerOpen, setDrawerItem);
  ...
  const { habitPackId,  } = route.params; //lessonId === "new" ? undefined : lessonId, 1
  const {
    loadMore,
    loadHabitPackHabits,
    basicParams,
    ...
    search,
    reSearch,
  } = useHabitPackHabitsSearchCollection(habitPackId === "new" ? undefined : habitPackId, 100, undefined, );
  ...
  return (
    <Drawer open={drawerOpen}  handleClose={handleClose} leftItem={
      <WebFadeIn off={!fadeIn} style={style}>
        <View style={[layoutStyles.page, layoutStyles.paddingMob20, cmsStyles?.habitPackHabitEditList, style]}>
              <ScrollView
                scrollEnabled={true}
              ...
              >
          <Header
            title={showTitle ? text("habitPackHabitEditList.title")[0] || "HabitPackHabits" : ""}
            // preamble={showPreamble ? text("habitPackHabitEditList.preamble")[0] || "All habitpackhabits" : ""}
            navigation={navigation}
            addNewDestination="HabitPackHabitEdit"
        ...
        <>
          {drawerItem === 0 && <HabitPackHabitEdit navigation={navigation} route={route} showHeader={false}/>}

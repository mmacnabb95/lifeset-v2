...
import { clearHabitCompletedRecordItems } from "src/redux/domain/features/habitCompletedRecord/collection-slice";
// import { deleteHabitCompletedRecord, updateHabitCompletedRecord } from "src/redux/domain/features/habitCompletedRecord/item-slice";
import { Habitcompletedrecord } from "../../../../../../types/domain/flat-types";
import { useHabitCompletedRecordCollection } from "src/redux/domain/features/habitCompletedRecord/useHabitCompletedRecordCollection";
import { initialLoadSize, rootLanguage } from "src/utils";
import { useHabitCompletedRecordsSearchCollection } from "src/redux/domain/features/habitCompletedRecord/useHabitCompletedRecordSearchCollection";
import { habitCompletedRecordsLoading } from "src/redux/domain/features/habitCompletedRecord/collection-slice";
...
const HabitCompletedRecordViewListScreen = ({
  navigation,
  route,
  showTitle = true,
  // showPreamble = true,
  ...
  scrollEnabled = true,
  destination = "HabitCompletedRecordView",
  ...
  const viewListConfig = useViewListConfig("HabitCompletedRecord", route);
  ...
  const {
    ...
    Filters,
  } = useHabitCompletedRecordsSearchCollection(initialLoadSize);
    ...
    <WebFadeIn off={!fadeIn} style={style} background={true}>
      <View
        style={[
          ...
          cmsStyles?.habitCompletedRecordViewList,
          style,
        ]}
      >
        <ScrollView
          ...
          contentContainerStyle={[
            layoutStyles.body,
            cmsStyles?.habitCompletedRecordViewListContentContainerStyle,
          ]}
        ...
        >
          <Header
            title={
              showTitle
                ? text("habitCompletedRecordViewList.title")[0] ||
                  "HabitCompletedRecords"
                : ""
            }
            preamble={text("habitCompletedRecordViewList.preamble")[0] || ""}
            ...
            style={[
              ...
              cmsStyles?.habitCompletedRecordViewListHeaderStyle,
            ]}
            titleStyle={cmsStyles?.habitCompletedRecordViewListTitleStyle}
          />
          ...
          <ListBody
            navigation={navigation}
            route={route}
            listItems={habitCompletedRecords}
            mode="view"
            loading={loading}
            style={style}
            ...
            loadMore={loadMore}
            doLoad={loadHabitCompletedRecords}
            destination={destination}
            orderBy={["Order"]}
            paramKey="habitCompletedRecordId"
...
export default HabitCompletedRecordViewListScreen;

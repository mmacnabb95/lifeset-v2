...
// import { deleteMyGratitudeJournal, updateMyGratitudeJournal } from "src/redux/domain/features/myGratitudeJournal/item-slice";
import { Mygratitudejournal } from "../../../../../../types/domain/flat-types";
import { useMyGratitudeJournalCollection } from "src/redux/domain/features/myGratitudeJournal/useMyGratitudeJournalCollection";
import { initialLoadSize, rootLanguage } from "src/utils";
import { useMyGratitudeJournalsSearchCollection } from "src/redux/domain/features/myGratitudeJournal/useMyGratitudeJournalSearchCollection";
import { myGratitudeJournalsLoading } from "src/redux/domain/features/myGratitudeJournal/collection-slice";
...
const MyGratitudeJournalEditListScreen = ({
  navigation,
  route,
  showTitle = true,
  // showPreamble = true,
  ...
  scrollEnabled = true,
  destination = "MyGratitudeJournalEdit",
}: {
  navigation: any;
  route?: any;
  showTitle?: boolean;
  ...
  const editListConfig = useEditListConfig("MyGratitudeJournal", route);
  ...
  const {
    loadMore,
    loadMyGratitudeJournals,
    basicParams,
    searchResult: myGratitudeJournals,
    Filters,
  } = useMyGratitudeJournalsSearchCollection(userId, initialLoadSize);
  ...
  const loading = useSelector(myGratitudeJournalsLoading);
    ...
    <WebFadeIn off={!fadeIn} style={style} background={true}>
      <View
        style={[
          ...
          cmsStyles?.myGratitudeJournalEditList,
          style,
        ]}
      >
        <ScrollView
          ...
          contentContainerStyle={[
            layoutStyles.body,
            cmsStyles?.myGratitudeJournalEditListContentContainerStyle,
          ]}
        ...
        >
          <Header
            title={
              showTitle
                ? text("myGratitudeJournalEditList.title")[0] ||
                  "My gratitude journals"
                : ""
            }
            preamble={text("myGratitudeJournalEditList.preamble")[0] || ""}
            navigation={navigation}
            addNewDestination={
              route.params?.viewOnly || newPosition === "bottom"
                ? undefined
                : "MyGratitudeJournalEdit"
            }
            addNewParams={{ myGratitudeJournalId: "new" }}
            hideGoBack={width < commonConstants.mobileBreak || hideGoBack}
            newEnabled={newEnabled}
            style={[
              style,
              layoutStyles.editListHeaderStyle,
              cmsStyles?.myGratitudeJournalEditListHeaderStyle,
            ]}
            titleStyle={[
              layoutStyles.parentListPageTitle,
              cmsStyles?.myGratitudeJournalEditListTitleStyle,
            ]}
          />
          ...
          <ListBody
            navigation={navigation}
            route={route}
            listItems={myGratitudeJournals}
            loading={loading}
            style={style}
            contentStyle={contentStyle}
            basicParams={basicParams}
            loadMore={loadMore}
            doLoad={loadMyGratitudeJournals}
            destination={destination}
            orderBy={["Order"]}
            paramKey="myGratitudeJournalId"
          />
        </ScrollView>
        {!route.params?.viewOnly && newPosition === "bottom" && (
          <AddNew
            destinationScreen={
              route.params?.viewOnly ? "" : "MyGratitudeJournalEdit"
            }
            params={{ myGratitudeJournalId: "new" }}
...
export default MyGratitudeJournalEditListScreen;

...
import { clearAllTimeStreakItems } from "src/redux/domain/features/allTimeStreak/collection-slice";
// import { deleteAllTimeStreak, updateAllTimeStreak } from "src/redux/domain/features/allTimeStreak/item-slice";
import { Alltimestreak } from "../../../../../../types/domain/flat-types";
import { useAllTimeStreakCollection } from "src/redux/domain/features/allTimeStreak/useAllTimeStreakCollection";
import { initialLoadSize, rootLanguage } from "src/utils";
import { useAllTimeStreaksSearchCollection } from "src/redux/domain/features/allTimeStreak/useAllTimeStreakSearchCollection";
import { allTimeStreaksLoading } from "src/redux/domain/features/allTimeStreak/collection-slice";
...
const AllTimeStreakViewListScreen = ({
  navigation,
  route,
  showTitle = true,
  // showPreamble = true,
  ...
  const viewListConfig = useViewListConfig("AllTimeStreak", route);
  ...
  const {
    ...
    loadAllTimeStreaks,
    basicParams,
    searchResult: allTimeStreaks,
    Filters,
  } = useAllTimeStreaksSearchCollection(userId, initialLoadSize);
  ...
  const loading = useSelector(allTimeStreaksLoading);
    ...
    <WebFadeIn off={!fadeIn} style={style} background={true}>
      <View
        style={[
          ...
          cmsStyles?.allTimeStreakViewList,
          style,
        ]}
      >
        <ScrollView
          ...
          contentContainerStyle={[
            layoutStyles.body,
            cmsStyles?.allTimeStreakViewListContentContainerStyle,
          ]}
        ...
        >
          <Header
            title={
              showTitle
                ? text("allTimeStreakViewList.title")[0] || "AllTimeStreaks"
                : ""
            }
            preamble={text("allTimeStreakViewList.preamble")[0] || ""}
            ...
            style={[
              ...
              cmsStyles?.allTimeStreakViewListHeaderStyle,
            ]}
            titleStyle={cmsStyles?.allTimeStreakViewListTitleStyle}
          />
          ...
          <ListBody
            ...
            loadMore={loadMore}
            doLoad={loadAllTimeStreaks}
            destination={destination}
            orderBy={["Order"]}
            paramKey="allTimeStreakId"
...
export default AllTimeStreakViewListScreen;

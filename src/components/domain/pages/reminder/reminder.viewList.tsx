...
import { clearReminderItems } from "src/redux/domain/features/reminder/collection-slice";
// import { deleteReminder, updateReminder } from "src/redux/domain/features/reminder/item-slice";
import { Reminder } from "../../../../../../types/domain/flat-types";
import { useReminderCollection } from "src/redux/domain/features/reminder/useReminderCollection";
import { initialLoadSize, rootLanguage } from "src/utils"
import { useRemindersSearchCollection } from "src/redux/domain/features/reminder/useReminderSearchCollection";
import { remindersLoading } from "src/redux/domain/features/reminder/collection-slice";
...
import TextSearch from "src/redux/domain/features/reminder/textSearch";
import { useUserInfo } from "src/redux/features/userInfo/useUserInfo";

const ReminderViewListScreen = ({
  navigation,
  route,
  showTitle = true,
  // showPreamble = true,
  ...
  const viewListConfig = useViewListConfig("Reminder", route);
  ...
  const {
    ...
    Filters
  } = useRemindersSearchCollection(initialLoadSize)
  ...
  const loading = useSelector(remindersLoading);
  ...
  return (
    <WebFadeIn off={!fadeIn} style={style} background={true}>
      <View style={[layoutStyles.page, layoutStyles.paddingMob20, layoutStyles.parentViewList, cmsStyles?.reminderViewList, style]}>
        <ScrollView
          scrollEnabled={scrollEnabled}
          nestedScrollEnabled={true}
          style={[layoutStyles.scrollViewContainer]}
          contentContainerStyle={[layoutStyles.body, cmsStyles?.reminderViewListContentContainerStyle ]}
          showsVerticalScrollIndicator={false}
          onScroll={scrollCallback}
          scrollEventThrottle={1000}
        >
        <Header
          title={showTitle ? text("reminderViewList.title")[0] || "Reminders" : ""}
          preamble={text("reminderViewList.preamble")[0] || ""}
          navigation={navigation}
          addNewDestination={undefined}
          
          hideGoBack={width < commonConstants.mobileBreak || hideGoBack}
          newEnabled={false}
          style={[style, layoutStyles.viewListHeaderStyle, cmsStyles?.reminderViewListHeaderStyle]}
          titleStyle={cmsStyles?.reminderViewListTitleStyle}
...
export default ReminderViewListScreen;

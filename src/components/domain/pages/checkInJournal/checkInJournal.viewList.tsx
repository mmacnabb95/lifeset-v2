...
// import { deleteCheckInJournal, updateCheckInJournal } from "src/redux/domain/features/checkInJournal/item-slice";
import { Checkinjournal } from "../../../../../../types/domain/flat-types";
import { useCheckInJournalCollection } from "src/redux/domain/features/checkInJournal/useCheckInJournalCollection";
import { initialLoadSize, rootLanguage } from "src/utils"
import { useCheckInJournalsSearchCollection } from "src/redux/domain/features/checkInJournal/useCheckInJournalSearchCollection";
import { checkInJournalsLoading } from "src/redux/domain/features/checkInJournal/collection-slice";
...
import TextSearch from "src/redux/domain/features/checkInJournal/textSearch";
import { useUserInfo } from "src/redux/features/userInfo/useUserInfo";

const CheckInJournalViewListScreen = ({
  navigation,
  route,
  showTitle = true,
  // showPreamble = true,
  ...
  scrollEnabled = true,
  destination = "CheckInJournalView",
  ...
  const viewListConfig = useViewListConfig("CheckInJournal", route);
  ...
  const dispatch = useDispatch();
    ...
    const {
      ...
      loadCheckInJournals,
      basicParams,
      searchResult: checkInJournals,
      Filters,
    } = useCheckInJournalsSearchCollection(userId, initialLoadSize);
  ...
  const loading = useSelector(checkInJournalsLoading);
  ...
  return (
    <WebFadeIn off={!fadeIn} style={style} background={true}>
      <View style={[layoutStyles.page, layoutStyles.paddingMob20, layoutStyles.parentViewList, cmsStyles?.checkInJournalViewList, style]}>
        <ScrollView
          scrollEnabled={scrollEnabled}
          nestedScrollEnabled={true}
          style={[layoutStyles.scrollViewContainer]}
          contentContainerStyle={[layoutStyles.body, cmsStyles?.checkInJournalViewListContentContainerStyle ]}
          showsVerticalScrollIndicator={false}
          onScroll={scrollCallback}
          scrollEventThrottle={1000}
        >
        <Header
          title={showTitle ? text("checkInJournalViewList.title")[0] || "CheckInJournals" : ""}
          preamble={text("checkInJournalViewList.preamble")[0] || ""}
          navigation={navigation}
          addNewDestination={undefined}
          
          hideGoBack={width < commonConstants.mobileBreak || hideGoBack}
          newEnabled={false}
          style={[style, layoutStyles.viewListHeaderStyle, cmsStyles?.checkInJournalViewListHeaderStyle]}
          titleStyle={cmsStyles?.checkInJournalViewListTitleStyle}
        ...
        <ListBody
          ...
          loadMore={loadMore}
          doLoad={loadCheckInJournals}
          destination={destination}
          orderBy={['Order', ]}
          paramKey="checkInJournalId"
...
export default CheckInJournalViewListScreen;

/* eslint-disable prettier/prettier */
/* eslint-disable no-trailing-spaces */
import { useFocusEffect } from "@react-navigation/native";
import React from "react";
import { View, Text } from "react-native";
import { useDispatch, useSelector } from "react-redux";
import { Slot } from "src/components/common/config/slot/slot";
import { videoPlayerFullScreen } from "src/redux/features/misc/slice";
import {
  userWorkoutExerciseSetLoading,
  getUserWorkoutExerciseSet,
  userWorkoutExerciseSetSelector,
} from "src/redux/domain/features/userWorkoutExerciseSet/collection-slice";

import { Userworkoutexerciseset } from "../../../../../../types/domain/flat-types";
import { useTranslation } from "src/translations/useTranslation";
import {useOnScrollContainerCloseToBottom} from 'src/utils'

import {Body, Header, Typography, WebFadeIn} from "src/components/common";
import UploadViewer from "src/lib/imagekit/screens/Upload/uploadViewer";
import {TypographyTypes} from "../../../common/typography";
const useCmsStyles = require("../../../../themes/cms/styles/styles").default;
const useLayoutStyles = require("../../../../themes/layout/styles/styles").default;
const useFormStyles = require("../../../../themes/form/styles/styles").default;

const UserWorkoutExerciseSetViewScreen = ({
  navigation,
  route,
}: {
  navigation: any;
  route: any;
}) => {
  const layoutStyles = useLayoutStyles();
  const cmsStyles = useCmsStyles();
  const formStyles = useFormStyles();

  const dispatch = useDispatch();
  const { text } = useTranslation();
  const { userWorkoutExerciseSetId } = route.params;
  const loading = useSelector(userWorkoutExerciseSetLoading);
  const userWorkoutExerciseSet: Userworkoutexerciseset = useSelector(
    userWorkoutExerciseSetSelector(userWorkoutExerciseSetId),
  );
  const videoFullScreen = useSelector(videoPlayerFullScreen);

  

  useFocusEffect(
    React.useCallback(() => {
      if (userWorkoutExerciseSetId !== "new") {
        dispatch(getUserWorkoutExerciseSet(userWorkoutExerciseSetId));
      }
    }, [dispatch, userWorkoutExerciseSetId]),
  );

  const {scrollCallback, isCloseToBottom} = useOnScrollContainerCloseToBottom();



  return (
    <WebFadeIn>
      <View style={[videoFullScreen ? layoutStyles.fullScreenPage : [layoutStyles.page, cmsStyles?.userWorkoutExerciseSet]]}>
        <Header style={[formStyles.form]} backButtonStyle={{}} navigation={navigation} />
        <Body  onScrollCallback={scrollCallback}>
          <View style={[formStyles.form, {flexGrow: 1}]}>
            {loading && !userWorkoutExerciseSet && <Text>{text("common.loading")}...</Text>}
            
            <View style={[videoFullScreen ? {} : [layoutStyles.itemPage]]}>
              <Slot
                index={'first'}
                componentName={"UserWorkoutExerciseSet"}
                navigation={navigation}
                route={route}
                source={userWorkoutExerciseSet}
              />
              <Typography type={TypographyTypes.H1} style={[cmsStyles.title, layoutStyles.title]} text={userWorkoutExerciseSet?.Title || userWorkoutExerciseSet?.Name} />
              
              { userWorkoutExerciseSet?.Introduction && (
                <Typography type={TypographyTypes.Body1} style={[cmsStyles.intro, layoutStyles.intro]} text={userWorkoutExerciseSet?.Introduction} />
              )}
              
              <Typography type={TypographyTypes.Body1} style={[cmsStyles.article, layoutStyles.article]} text={userWorkoutExerciseSet?.Article || userWorkoutExerciseSet?.Description || userWorkoutExerciseSet?.Text} />
              <Slot
                index={'last'}
                componentName={"UserWorkoutExerciseSet"}
                navigation={navigation}
                route={route}
                source={userWorkoutExerciseSet}
                parentPageIsCloseToBottomOfScroll={isCloseToBottom}
              />
            </View>
          </View>
        </Body>
      </View>
    </WebFadeIn>
  );
};

export default UserWorkoutExerciseSetViewScreen;



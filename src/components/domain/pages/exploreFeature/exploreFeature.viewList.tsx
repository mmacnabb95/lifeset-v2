...
import { clearExploreFeatureItems } from "src/redux/domain/features/exploreFeature/collection-slice";
// import { deleteExploreFeature, updateExploreFeature } from "src/redux/domain/features/exploreFeature/item-slice";
import { Explorefeature } from "../../../../../../types/domain/flat-types";
import { useExploreFeatureCollection } from "src/redux/domain/features/exploreFeature/useExploreFeatureCollection";
import { initialLoadSize, rootLanguage } from "src/utils";
import { useExploreFeaturesSearchCollection } from "src/redux/domain/features/exploreFeature/useExploreFeatureSearchCollection";
import { exploreFeaturesLoading } from "src/redux/domain/features/exploreFeature/collection-slice";
...
const ExploreFeatureViewListScreen = ({
  navigation,
  route,
  showTitle = true,
  // showPreamble = true,
  ...
  scrollEnabled = true,
  destination = "ExploreFeatureView",
  ...
  const viewListConfig = useViewListConfig("ExploreFeature", route);
  ...
  const {
    ...
    loadExploreFeatures,
    basicParams,
    searchResult: exploreFeatures,
    Filters,
  } = useExploreFeaturesSearchCollection(userId, initialLoadSize);
  ...
  const loading = useSelector(exploreFeaturesLoading);
    ...
    <WebFadeIn off={!fadeIn} style={style} background={true}>
      <View
        style={[
          ...
          cmsStyles?.exploreFeatureViewList,
          style,
        ]}
      >
        <ScrollView
          ...
          contentContainerStyle={[
            layoutStyles.body,
            cmsStyles?.exploreFeatureViewListContentContainerStyle,
          ]}
        ...
        >
          <Header
            title={
              showTitle
                ? text("exploreFeatureViewList.title")[0] || "ExploreFeatures"
                : ""
            }
            preamble={text("exploreFeatureViewList.preamble")[0] || ""}
            ...
            style={[
              ...
              cmsStyles?.exploreFeatureViewListHeaderStyle,
            ]}
            titleStyle={cmsStyles?.exploreFeatureViewListTitleStyle}
          />
          ...
          <ListBody
            ...
            loadMore={loadMore}
            doLoad={loadExploreFeatures}
            destination={destination}
            orderBy={["Order"]}
            paramKey="exploreFeatureId"
...
export default ExploreFeatureViewListScreen;

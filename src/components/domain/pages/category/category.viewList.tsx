...
import { clearCategoryItems } from "src/redux/domain/features/category/collection-slice";
// import { deleteCategory, updateCategory } from "src/redux/domain/features/category/item-slice";
import { Category } from "../../../../../../types/domain/flat-types";
import { useCategoryCollection } from "src/redux/domain/features/category/useCategoryCollection";
import { initialLoadSize, rootLanguage } from "src/utils"
import { useCategorysSearchCollection } from "src/redux/domain/features/category/useCategorySearchCollection";
import { categorysLoading } from "src/redux/domain/features/category/collection-slice";
...
import TextSearch from "src/redux/domain/features/category/textSearch";
import { useUserInfo } from "src/redux/features/userInfo/useUserInfo";

const CategoryViewListScreen = ({
  navigation,
  route,
  showTitle = true,
  // showPreamble = true,
  ...
  const viewListConfig = useViewListConfig("Category", route);
  ...
  const {
    ...
    loadCategorys,
    basicParams,
    searchResult: categorys,
    Filters
  } = useCategorysSearchCollection(initialLoadSize)
  ...
  const loading = useSelector(categorysLoading);
  ...
  return (
    <WebFadeIn off={!fadeIn} style={style} background={true}>
      <View style={[layoutStyles.page, layoutStyles.paddingMob20, layoutStyles.parentViewList, cmsStyles?.categoryViewList, style]}>
        <ScrollView
          scrollEnabled={scrollEnabled}
          nestedScrollEnabled={true}
          style={[layoutStyles.scrollViewContainer]}
          contentContainerStyle={[layoutStyles.body, cmsStyles?.categoryViewListContentContainerStyle ]}
          showsVerticalScrollIndicator={false}
          onScroll={scrollCallback}
          scrollEventThrottle={1000}
        >
        <Header
          title={showTitle ? text("categoryViewList.title")[0] || "Categorys" : ""}
          preamble={text("categoryViewList.preamble")[0] || ""}
          navigation={navigation}
          addNewDestination={undefined}
          
          hideGoBack={width < commonConstants.mobileBreak || hideGoBack}
          newEnabled={false}
          style={[style, layoutStyles.viewListHeaderStyle, cmsStyles?.categoryViewListHeaderStyle]}
          titleStyle={cmsStyles?.categoryViewListTitleStyle}
...
export default CategoryViewListScreen;

...
import { clearHabitResourceItems } from "src/redux/domain/features/habitResource/collection-slice";
// import { deleteHabitResource, updateHabitResource } from "src/redux/domain/features/habitResource/item-slice";
import { Habitresource } from "../../../../../../types/domain/flat-types";
import { useHabitResourceCollection } from "src/redux/domain/features/habitResource/useHabitResourceCollection";
import { initialLoadSize, rootLanguage } from "src/utils";
import { useHabitResourcesSearchCollection } from "src/redux/domain/features/habitResource/useHabitResourceSearchCollection";
import { habitResourcesLoading } from "src/redux/domain/features/habitResource/collection-slice";
...
const HabitResourceViewListScreen = ({
  navigation,
  route,
  showTitle = true,
  // showPreamble = true,
  ...
  const viewListConfig = useViewListConfig("HabitResource", route);
  ...
  const {
    ...
    Filters,
  } = useHabitResourcesSearchCollection(initialLoadSize);
    ...
    <WebFadeIn off={!fadeIn} style={style} background={true}>
      <View
        style={[
          ...
          cmsStyles?.habitResourceViewList,
          style,
        ]}
      >
        <ScrollView
          ...
          contentContainerStyle={[
            layoutStyles.body,
            cmsStyles?.habitResourceViewListContentContainerStyle,
          ]}
        ...
        >
          <Header
            title={
              showTitle
                ? text("habitResourceViewList.title")[0] || "HabitResources"
                : ""
            }
            preamble={text("habitResourceViewList.preamble")[0] || ""}
            ...
            style={[
              ...
              cmsStyles?.habitResourceViewListHeaderStyle,
            ]}
            titleStyle={cmsStyles?.habitResourceViewListTitleStyle}
          />
          ...
          <ListBody
            ...
            loadMore={loadMore}
            doLoad={loadHabitResources}
            destination={destination}
            orderBy={["Order"]}
            paramKey="habitResourceId"
...
export default HabitResourceViewListScreen;

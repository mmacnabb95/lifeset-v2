...
import TextSearch from "src/redux/domain/features/journal/textSearch";
...
const JournalEditListScreen = ({
  ...
  const editListConfig = useEditListConfig("Journal", route);
  ...
  const dispatch = useDispatch();
    ...
    const {
      ...
      Filters,
    } = useJournalsSearchCollection(userId, initialLoadSize);
  ...
  return (
    <WebFadeIn off={!fadeIn} style={style} background={true}>
      <View style={[layoutStyles.page, layoutStyles.paddingMob20, layoutStyles.parentEditList, cmsStyles?.journalEditList, style]}>
        <ScrollView
          scrollEnabled={scrollEnabled}
          nestedScrollEnabled={true}
          style={[layoutStyles.scrollViewContainer]}
          contentContainerStyle={[layoutStyles.body,cmsStyles?.journalEditListContentContainerStyle ]}
        ...
        <Header
          ...
          preamble={text("journalEditList.preamble")[0] || ""}
          navigation={navigation}
          addNewDestination={route.params?.viewOnly || newPosition === "bottom" ? undefined :"JournalEdit"}
          addNewParams={{ journalId: "new" }}
          
          hideGoBack={width < commonConstants.mobileBreak || hideGoBack}
          newEnabled={newEnabled}
          style={[style, layoutStyles.editListHeaderStyle, cmsStyles?.journalEditListHeaderStyle]}
          titleStyle={[layoutStyles.parentListPageTitle, cmsStyles?.journalEditListTitleStyle]}
        ...
        {!route.params?.viewOnly && newPosition === "bottom" && (
          <AddNew
            destinationScreen={route.params?.viewOnly ? "" : "JournalEdit"}
            params={{ journalId: "new" }}

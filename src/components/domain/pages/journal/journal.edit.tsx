...
import JournalForm, { defaultTextValues } from "src/components/domain/journal/journal.form";
...
import { clearJournalItems, createJournal, updateJournal } from "src/redux/domain/features/journal/collection-slice";
import {
  getJournal,
  journalLoading,
  journalSelector,
} from "src/redux/domain/features/journal/collection-slice";
...
const JournalEditScreen = React.forwardRef(({
  navigation,
  route,
  showHeader = true,
  hideButtons = false,
  ...
  const { viewOnly, journalId,  } = route.params;
  const journal: Journal = useSelector(
    journalSelector(journalId === "new" ? -1 : journalId),
  );
  const loading = useSelector(journalLoading);
  const [wasNew, setWasNew] = useState(journalId === "new");
  ...
  const formConfig = getConfig("Journal", journal, formRef, navigation, route);

  useFocusEffect(
    React.useCallback(() => {
      if (journalId !== "new" && wasNew === false) {
         dispatch(getJournal(journalId));
      }
    }, [dispatch, journalId, wasNew]),
  );
  ...
  getAppendees("Journal", fieldAppendees, journal, formRef, navigation, route);
  ...
  return (
    <>
      <WebFadeIn background={true}>
        <View style={[[layoutStyles.page, layoutStyles.parentEditPage, cmsStyles?.journalEditPage]]}>
          {showHeader && <Header navigation={navigation}/>}
          <Body contentStyle={[cmsStyles?.journalEditPageBody]}>
            <>
              
              {(journalId === "new" || (journal !== undefined && journal !== "")) && (
                <JournalForm
                  ref={formRef}
                  key={journalId}
                  fieldAppendees={fieldAppendees}
                  formItem={journal || formConfig?.default || {User: userId, }}
                  viewOnly={viewOnly}
                  ...
                  afterCreate={(response: any) => {
                    if (!handleError(response)) {
                      ...
                      navigation.setParams({journalId: response.payload.Id});
                      setAlertTitle(t("journal.onCreated") || "Journal created");
                      setAlertIcon("check");
                  ...
                  afterUpdate={(response: any) => {
                    if (!handleError(response)) {
                      setAlertTitle(t("journal.onUpdated") || "Journal updated");
                      setAlertIcon("check");
                  ...
                  afterDelete={(response) => {
                    if (!handleError(response)) {
                      setAlertTitle(t("journal.onDeleted") || "Journal deleted");
                      setAlertIcon("check");
                      setIsAlertOpen(true);
              ...
              {(loading && (journal === undefined || journal === "")) &&
                journalId !== "new" && (
                  <View>
      ...
      <Modal
        ...
        title={t("journal.deleteWarningTitle") || "Delete Journal"}
        text={t("journal.deleteWarningText") || "Are you sure you want to delete this Journal?"}
        acceptButton={(
          <Button
            title="Delete"
            onPress={() => {

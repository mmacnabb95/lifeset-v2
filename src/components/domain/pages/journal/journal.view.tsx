...
const JournalViewScreen = ({
  ...
  return (
    <WebFadeIn>
      <View style={[videoFullScreen ? layoutStyles.fullScreenPage : [layoutStyles.page, cmsStyles?.journal]]}>
        ...
        <Body  onScrollCallback={scrollCallback}>
          <View style={[formStyles.form, {flexGrow: 1}]}>
            {loading && !journal && <Text>{text("common.loading")}...</Text>}
            ...
            <View style={[videoFullScreen ? {} : [layoutStyles.itemPage]]}>
              <Slot
                ...
                source={journal}
              />
              <Typography type={TypographyTypes.H1} style={[cmsStyles.title, layoutStyles.title]} text={journal?.Title || journal?.Name} />
              
              { journal?.Introduction && (
                <Typography type={TypographyTypes.Body1} style={[cmsStyles.intro, layoutStyles.intro]} text={journal?.Introduction} />
              )}
              
              <Typography type={TypographyTypes.Body1} style={[cmsStyles.article, layoutStyles.article]} text={journal?.Article || journal?.Description || journal?.Text} />
              <Slot
                index={'last'}
                componentName={"Journal"}
                navigation={navigation}

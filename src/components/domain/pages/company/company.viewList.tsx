...
const CompanyViewListScreen = ({
  ...
  const viewListConfig = useViewListConfig("Company", route);
  ...
  const getIconPath = (company: Company) => {
    const iconPath = company?.resources?.find((r: any) => {
      try {
        const meta = JSON.parse(r?.Meta);
        return meta?.fileType === "image";
      } catch (e) {
        console.error("Invalid JSON in resource Meta:", r?.Meta, e);
        return false;
      }
    })?.Url;
      ...
      >
        <ScrollView
          ...
          contentContainerStyle={[
            layoutStyles.body,
            cmsStyles?.companyViewListContentContainerStyle,
          ]}
        ...
        >
          <Header
            title={
              showTitle ? text("companyViewList.title")[0] || "Companys" : ""
            }
            preamble={text("companyViewList.preamble")[0] || ""}
            ...
            style={[
              ...
              cmsStyles?.companyViewListHeaderStyle,
            ]}
            titleStyle={cmsStyles?.companyViewListTitleStyle}
            underHeaderComponents={
              <View style={layoutStyles.searchSection}>
                ...
                {filterTargetHeight > 0 && (
                  ...
                  >
                    <Icon iconType="filter" iconColor="white" iconSize={30} />
                  </Pressable>
                )}
              </View>
            }
...
export default CompanyViewListScreen;

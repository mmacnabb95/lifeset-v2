...
import HabitPackEdit from "src/components/domain/pages/habitPack/habitPack.edit"
...
import { useHabitPackInlineEditListDrawerHandler } from "src/components/common/config/hooks/useHabitPackInlineEditListDrawerHandler";
...
const HabitPackEditListScreen = ({
  ...
  const {onOpen, onClose} = useHabitPackInlineEditListDrawerHandler(setDrawerOpen, setDrawerItem);
  ...
  const {
    ...
    search,
    reSearch,
  } = useHabitPacksSearchCollection(companyId === "new" ? undefined : companyId, 100, undefined, );
  ...
  return (
    <Drawer open={drawerOpen}  handleClose={handleClose} leftItem={
      <WebFadeIn off={!fadeIn} style={style}>
        <View style={[layoutStyles.page, layoutStyles.paddingMob20, cmsStyles?.habitPackEditList, style]}>
              <ScrollView
                scrollEnabled={true}
              ...
              >
          <Header
            title={showTitle ? text("habitPackEditList.title")[0] || "HabitPacks" : ""}
            // preamble={showPreamble ? text("habitPackEditList.preamble")[0] || "All habitpacks" : ""}
            navigation={navigation}
            addNewDestination="HabitPackEdit"
        ...
        <>
          {drawerItem === 0 && <HabitPackEdit navigation={navigation} route={route} showHeader={false}/>}

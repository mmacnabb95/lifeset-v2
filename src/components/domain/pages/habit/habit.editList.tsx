...
const HabitEditListScreen = ({
  ...
  const editListConfig = useEditListConfig("Habit", route);
  ...
  const dispatch = useDispatch();
    ...
    const {
      ...
      searchResult: habits,
      Filters,
    } = useHabitsSearchCollection(userId, initialLoadSize);
  ...
  return (
    <WebFadeIn off={!fadeIn} style={style} background={true}>
      <View style={[layoutStyles.page, layoutStyles.paddingMob20, layoutStyles.parentEditList, cmsStyles?.habitEditList, style]}>
        <ScrollView
          scrollEnabled={scrollEnabled}
          nestedScrollEnabled={true}
          style={[layoutStyles.scrollViewContainer]}
          contentContainerStyle={[layoutStyles.body,cmsStyles?.habitEditListContentContainerStyle ]}
        ...
        <Header
          ...
          preamble={text("habitEditList.preamble")[0] || ""}
          navigation={navigation}
          addNewDestination={route.params?.viewOnly || newPosition === "bottom" ? undefined :"HabitEdit"}
          addNewParams={{ habitId: "new" }}
          
          hideGoBack={width < commonConstants.mobileBreak || hideGoBack}
          newEnabled={newEnabled}
          style={[style, layoutStyles.editListHeaderStyle, cmsStyles?.habitEditListHeaderStyle]}
          titleStyle={[layoutStyles.parentListPageTitle, cmsStyles?.habitEditListTitleStyle]}
        ...
        {!route.params?.viewOnly && newPosition === "bottom" && (
          <AddNew
            destinationScreen={route.params?.viewOnly ? "" : "HabitEdit"}
            params={{ habitId: "new" }}

...
import { clearStreakLeaderBoardItems } from "src/redux/domain/features/streakLeaderBoard/collection-slice";
// import { deleteStreakLeaderBoard, updateStreakLeaderBoard } from "src/redux/domain/features/streakLeaderBoard/item-slice";
import { Streakleaderboard } from "../../../../../../types/domain/flat-types";
import { useStreakLeaderBoardCollection } from "src/redux/domain/features/streakLeaderBoard/useStreakLeaderBoardCollection";
...
import { streakLeaderBoardsLoading } from "src/redux/domain/features/streakLeaderBoard/collection-slice";
...
import TextSearch from "src/redux/domain/features/streakLeaderBoard/textSearch";
import { useUserInfo } from "src/redux/features/userInfo/useUserInfo";

const StreakLeaderBoardViewListScreen = ({
  navigation,
  route,
  showTitle = true,
  // showPreamble = true,
  ...
  scrollEnabled = true,
  destination = "StreakLeaderBoardView",
  ...
  const viewListConfig = useViewListConfig("StreakLeaderBoard", route);
  ...
  const {
    ...
    basicParams,
    searchResult: streakLeaderBoards,
    Filters
  } = useStreakLeaderBoardsSearchCollection(initialLoadSize)
  ...
  const loading = useSelector(streakLeaderBoardsLoading);
  ...
  return (
    <WebFadeIn off={!fadeIn} style={style} background={true}>
      <View style={[layoutStyles.page, layoutStyles.paddingMob20, layoutStyles.parentViewList, cmsStyles?.streakLeaderBoardViewList, style]}>
        <ScrollView
          scrollEnabled={scrollEnabled}
          nestedScrollEnabled={true}
          style={[layoutStyles.scrollViewContainer]}
          contentContainerStyle={[layoutStyles.body, cmsStyles?.streakLeaderBoardViewListContentContainerStyle ]}
          showsVerticalScrollIndicator={false}
          onScroll={scrollCallback}
          scrollEventThrottle={1000}
        >
        <Header
          title={showTitle ? text("streakLeaderBoardViewList.title")[0] || "StreakLeaderBoards" : ""}
          preamble={text("streakLeaderBoardViewList.preamble")[0] || ""}
          navigation={navigation}
          addNewDestination={undefined}
          
          hideGoBack={width < commonConstants.mobileBreak || hideGoBack}
          newEnabled={false}
          style={[style, layoutStyles.viewListHeaderStyle, cmsStyles?.streakLeaderBoardViewListHeaderStyle]}
          titleStyle={cmsStyles?.streakLeaderBoardViewListTitleStyle}
          underHeaderComponents={
            <View style={layoutStyles.searchSection}>
            <TextSearch
              navigation={navigation}
              loadStreakLeaderBoards={loadStreakLeaderBoards}
        ...
        <ListBody
          ...
          loadMore={loadMore}
          doLoad={loadStreakLeaderBoards}
          destination={destination}
          orderBy={['Order',  'MaxStreakThisMonth desc']}
          paramKey="streakLeaderBoardId"
...
export default StreakLeaderBoardViewListScreen;

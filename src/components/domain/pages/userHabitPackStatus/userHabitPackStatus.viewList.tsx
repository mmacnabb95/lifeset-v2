...
import { clearUserHabitPackStatusItems } from "src/redux/domain/features/userHabitPackStatus/collection-slice";
// import { deleteUserHabitPackStatus, updateUserHabitPackStatus } from "src/redux/domain/features/userHabitPackStatus/item-slice";
import { Userhabitpackstatus } from "../../../../../../types/domain/flat-types";
import { useUserHabitPackStatusCollection } from "src/redux/domain/features/userHabitPackStatus/useUserHabitPackStatusCollection";
import { initialLoadSize, rootLanguage } from "src/utils";
import { useUserHabitPackStatussSearchCollection } from "src/redux/domain/features/userHabitPackStatus/useUserHabitPackStatusSearchCollection";
import { userHabitPackStatussLoading } from "src/redux/domain/features/userHabitPackStatus/collection-slice";
...
const UserHabitPackStatusViewListScreen = ({
  navigation,
  route,
  showTitle = true,
  // showPreamble = true,
  ...
  scrollEnabled = true,
  destination = "UserHabitPackStatusView",
  ...
  const viewListConfig = useViewListConfig("UserHabitPackStatus", route);
  ...
  const {
    loadMore,
    loadUserHabitPackStatuss,
    basicParams,
    searchResult: userHabitPackStatuss,
    Filters,
  } = useUserHabitPackStatussSearchCollection(initialLoadSize);
  ...
  const loading = useSelector(userHabitPackStatussLoading);
    ...
    <WebFadeIn off={!fadeIn} style={style} background={true}>
      <View
        style={[
          ...
          cmsStyles?.userHabitPackStatusViewList,
          style,
        ]}
      >
        <ScrollView
          ...
          contentContainerStyle={[
            layoutStyles.body,
            cmsStyles?.userHabitPackStatusViewListContentContainerStyle,
          ]}
        ...
        >
          <Header
            title={
              showTitle
                ? text("userHabitPackStatusViewList.title")[0] ||
                  "UserHabitPackStatuss"
                : ""
            }
            preamble={text("userHabitPackStatusViewList.preamble")[0] || ""}
            ...
            style={[
              ...
              cmsStyles?.userHabitPackStatusViewListHeaderStyle,
            ]}
            titleStyle={cmsStyles?.userHabitPackStatusViewListTitleStyle}
          />
          ...
          <ListBody
            navigation={navigation}
            route={route}
            listItems={userHabitPackStatuss}
            mode="view"
            loading={loading}
            style={style}
            ...
            loadMore={loadMore}
            doLoad={loadUserHabitPackStatuss}
            destination={destination}
            orderBy={["Order"]}
            paramKey="userHabitPackStatusId"
...
export default UserHabitPackStatusViewListScreen;

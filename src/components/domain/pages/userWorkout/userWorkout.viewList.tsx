...
import { clearUserWorkoutItems } from "src/redux/domain/features/userWorkout/collection-slice";
// import { deleteUserWorkout, updateUserWorkout } from "src/redux/domain/features/userWorkout/item-slice";
import { Userworkout } from "../../../../../../types/domain/flat-types";
import { useUserWorkoutCollection } from "src/redux/domain/features/userWorkout/useUserWorkoutCollection";
...
import { userWorkoutsLoading } from "src/redux/domain/features/userWorkout/collection-slice";
...
import TextSearch from "src/redux/domain/features/userWorkout/textSearch";
import { useUserInfo } from "src/redux/features/userInfo/useUserInfo";

const UserWorkoutViewListScreen = ({
  navigation,
  route,
  showTitle = true,
  // showPreamble = true,
  ...
  const viewListConfig = useViewListConfig("UserWorkout", route);
  ...
  const dispatch = useDispatch();
    ...
    const {
      ...
      loadUserWorkouts,
      basicParams,
      searchResult: userWorkouts,
      Filters,
    } = useUserWorkoutsSearchCollection(userId, initialLoadSize);
  ...
  const loading = useSelector(userWorkoutsLoading);
  ...
  return (
    <WebFadeIn off={!fadeIn} style={style} background={true}>
      <View style={[layoutStyles.page, layoutStyles.paddingMob20, layoutStyles.parentViewList, cmsStyles?.userWorkoutViewList, style]}>
        <ScrollView
          scrollEnabled={scrollEnabled}
          nestedScrollEnabled={true}
          style={[layoutStyles.scrollViewContainer]}
          contentContainerStyle={[layoutStyles.body, cmsStyles?.userWorkoutViewListContentContainerStyle ]}
          showsVerticalScrollIndicator={false}
          onScroll={scrollCallback}
          scrollEventThrottle={1000}
        >
        <Header
          title={showTitle ? text("userWorkoutViewList.title")[0] || "UserWorkouts" : ""}
          preamble={text("userWorkoutViewList.preamble")[0] || ""}
          navigation={navigation}
          addNewDestination={undefined}
          
          hideGoBack={width < commonConstants.mobileBreak || hideGoBack}
          newEnabled={false}
          style={[style, layoutStyles.viewListHeaderStyle, cmsStyles?.userWorkoutViewListHeaderStyle]}
          titleStyle={cmsStyles?.userWorkoutViewListTitleStyle}
          underHeaderComponents={
            <View style={layoutStyles.searchSection}>
            <TextSearch
              navigation={navigation}
              loadUserWorkouts={loadUserWorkouts}
        ...
        <ListBody
          ...
          loadMore={loadMore}
          doLoad={loadUserWorkouts}
          destination={destination}
          orderBy={['Order', ]}
          paramKey="userWorkoutId"
...
export default UserWorkoutViewListScreen;

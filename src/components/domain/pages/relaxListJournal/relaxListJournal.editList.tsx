...
const RelaxListJournalEditListScreen = ({
  navigation,
  route,
  showTitle = true,
  // showPreamble = true,
  ...
  scrollEnabled = true,
  destination = "RelaxListJournalEdit",
}: {
  navigation: any;
  route?: any;
  showTitle?: boolean;
  ...
  const editListConfig = useEditListConfig("RelaxListJournal", route);
    ...
    <WebFadeIn off={!fadeIn} style={style} background={true}>
      <View
        style={[
          ...
          cmsStyles?.relaxListJournalEditList,
          style,
        ]}
      >
        <ScrollView
          ...
          contentContainerStyle={[
            layoutStyles.body,
            cmsStyles?.relaxListJournalEditListContentContainerStyle,
          ]}
        ...
        >
          <Header
            title={
              showTitle
                ? text("relaxListJournalEditList.title")[0] ||
                  "Relax list journals"
                : ""
            }
            preamble={text("relaxListJournalEditList.preamble")[0] || ""}
            navigation={navigation}
            addNewDestination={
              route.params?.viewOnly || newPosition === "bottom"
                ? undefined
                : "RelaxListJournalEdit"
            }
            addNewParams={{ relaxListJournalId: "new" }}
            hideGoBack={width < commonConstants.mobileBreak || hideGoBack}
            newEnabled={newEnabled}
            style={[
              style,
              layoutStyles.editListHeaderStyle,
              cmsStyles?.relaxListJournalEditListHeaderStyle,
            ]}
            titleStyle={[
              layoutStyles.parentListPageTitle,
              cmsStyles?.relaxListJournalEditListTitleStyle,
            ]}
          />
        ...
        {!route.params?.viewOnly && newPosition === "bottom" && (
          <AddNew
            destinationScreen={
              route.params?.viewOnly ? "" : "RelaxListJournalEdit"
            }
            params={{ relaxListJournalId: "new" }}
...
export default RelaxListJournalEditListScreen;

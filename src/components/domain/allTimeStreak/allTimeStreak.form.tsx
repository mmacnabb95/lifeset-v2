...
import { createAllTimeStreak, updateAllTimeStreak, deleteAllTimeStreak, allTimeStreakErrorSelector, allTimeStreakSelector , clearAllTimeStreakError } from "src/redux/domain/features/allTimeStreak/collection-slice";
...
export const defaultTextValues:DefaultTextValues[] = [
  ...
  { propertyName : "StreakDays", label : "Streak days" , placeholder : "Streak days"}, 
];

const AllTimeStreakForm = forwardRef(({
  customTextValues,
  afterCreate,
  ...
  formItem?: Partial<Alltimestreak>,
  ...
  handleValuesChange?: ({
    ...
    values: Partial<Alltimestreak>,
    errors: FormikErrors<Alltimestreak>
  }) => void,
  ...
  const error = useSelector(allTimeStreakErrorSelector);
  const allTimeStreakItem = useSelector(allTimeStreakSelector(formItem?.Id));
  const [currentTextValues, setCurrentTextValues] = useState(defaultTextValues);

  
  const initialValues: Partial<Alltimestreak> = {
      User: allTimeStreakItem?.User || formItem?.User || undefined,
      StreakDays: allTimeStreakItem?.StreakDays || formItem?.StreakDays || undefined,
      UpdatedDate: allTimeStreakItem?.UpdatedDate || formItem?.UpdatedDate || undefined,
      IncrementedDate: allTimeStreakItem?.IncrementedDate || formItem?.IncrementedDate || undefined,
  };
  ...
  const itemExists = () => {
    return allTimeStreakItem || (formItem && formItem?.Id && formItem.Id > 0) ? true : false;
  };

  const getItemId = () => {
    if (allTimeStreakItem?.Id) return { Id: allTimeStreakItem?.Id };
    return {};
  }

  useImperativeHandle(ref, () => (formik));

  const formik = useFormik<Partial<Alltimestreak>>({
    initialValues: { ...initialValues, ...formItem },
    validationSchema: Yup.object({
        User: Yup.string().required(textValues?.requiredText || 'Required'),
        StreakDays: Yup.string().required(textValues?.requiredText || 'Required'),
        ...injectedValidationSchema,
    }),
    onSubmit: async (allTimeStreak) => {
      let response:any;
      if (itemExists()) {
        const updateObjectFull = { ...allTimeStreak, ...getItemId() } ;
        const updateObjectIgnored = _.omit(updateObjectFull, ["HasVideo", "HasImage", ]);
        response = await dispatch(updateAllTimeStreak(updateObjectIgnored));

        if(afterUpdate) afterUpdate({...response, isValid});
      } else {
        response = await dispatch(createAllTimeStreak(_.omit(formik.values, ["HasVideo", "HasImage"])));
  ...
  const handleDelete = async () => {
      if (!beforeDelete || await beforeDelete() === true) {
        const response: any = await dispatch(deleteAllTimeStreak(allTimeStreakItem?.Id as number));
  ...
  useFocusEffect(
    React.useCallback(() => {
      dispatch(clearAllTimeStreakError());
    }, [dispatch]),
  );

  return (
    <View style={[formStyles.form, layoutStyles.parentForm, cmsStyles?.allTimeStreakForm, style?.form]}>
      ...
      { buttonsTop && (
          <View style={[formStyles.buttons, style?.buttons]}>
            <Button
              ...
              testID={"submit-AllTimeStreak-form"}
              title={itemExists() ? updateButtonText ? updateButtonText : `Update All Time Streak` : saveButtonText ? saveButtonText : `Save All Time Streak`}
            ...
            {!hideDeleteButton && itemExists() && (
              <Button
                ...
                testID={"deleteAllTimeStreak"}
                title={deleteButtonText ? deleteButtonText : `Delete All Time Streak`}
          ...
          <Input
            editable={viewOnly === true ? false : (readOnly && readOnly.indexOf('StreakDays') !== -1 ? false : true)}
            multiline={fieldConfig['StreakDays']?.multiline ? fieldConfig['StreakDays'].multiline  : windowWidth > mobileBreak ? false : false}
            numberOfLines={fieldConfig['StreakDays']?.numberOfLines ? fieldConfig['StreakDays'].numberOfLines : 1}
            // scrollEnabled={false}
            inputLabelStyle={[
              style?.inputLabelStyle
            ]}
            ...
            testID="StreakDays"
            placeholder={ returnCorrectTextValue("StreakDays", "placeholder") }
            onChangeText={(text) => {
              formik.setFieldValue("StreakDays", text.replace(/[^0-9.]/g, ''), true);
              
            }}
            onBlur={formik.handleBlur("StreakDays")}
            value={formik.values.StreakDays?.toString()  || ""}
            onKeyPress={handleKeyDown}
            label={ returnCorrectTextValue("StreakDays", "label") }
            errorMessage={formik.touched.StreakDays && formik.errors.StreakDays}
            errorPlace={'centerRight'}
            
          />

        
          {fieldAppendees?.filter((f) => f.fieldName === 'StreakDays').map((fa) => {
            return fa.el;
          })}
...
export default AllTimeStreakForm;

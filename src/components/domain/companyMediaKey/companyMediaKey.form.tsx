...
import { createCompanyMediaKey, updateCompanyMediaKey, deleteCompanyMediaKey, companyMediaKeyErrorSelector, companyMediaKeySelector , clearCompanyMediaKeyError } from "src/redux/domain/features/companyMediaKey/collection-slice";
...
const CompanyMediaKeyForm = forwardRef(({
  customTextValues,
  afterCreate,
  ...
  formItem?: Partial<Companymediakey>,
  ...
  handleValuesChange?: ({
    ...
    values: Partial<Companymediakey>,
    errors: FormikErrors<Companymediakey>
  }) => void,
  ...
  const error = useSelector(companyMediaKeyErrorSelector);
  const companyMediaKeyItem = useSelector(companyMediaKeySelector(formItem?.Id));
  const [currentTextValues, setCurrentTextValues] = useState(defaultTextValues);

  
  const initialValues: Partial<Companymediakey> = {
      Key: companyMediaKeyItem?.Key || formItem?.Key || '',
      MediaRestriction: companyMediaKeyItem?.MediaRestriction || formItem?.MediaRestriction || '',
      Multiple: companyMediaKeyItem?.Multiple || formItem?.Multiple || false,
  };
  ...
  const itemExists = () => {
    return companyMediaKeyItem || (formItem && formItem?.Id && formItem.Id > 0) ? true : false;
  };

  const getItemId = () => {
    if (companyMediaKeyItem?.Id) return { Id: companyMediaKeyItem?.Id };
    return {};
  }

  useImperativeHandle(ref, () => (formik));

  const formik = useFormik<Partial<Companymediakey>>({
    initialValues: { ...initialValues, ...formItem },
    validationSchema: Yup.object({
        Key: Yup.string().required(textValues?.requiredText || 'Required'),
        ...injectedValidationSchema,
    }),
    onSubmit: async (companyMediaKey) => {
      let response:any;
      if (itemExists()) {
        const updateObjectFull = { ...companyMediaKey, ...getItemId() } ;
        const updateObjectIgnored = _.omit(updateObjectFull, ["HasVideo", "HasImage", ]);
        response = await dispatch(updateCompanyMediaKey(updateObjectIgnored));

        if(afterUpdate) afterUpdate({...response, isValid});
      } else {
        response = await dispatch(createCompanyMediaKey(_.omit(formik.values, ["HasVideo", "HasImage"])));
  ...
  const handleDelete = async () => {
      if (!beforeDelete || await beforeDelete() === true) {
        const response: any = await dispatch(deleteCompanyMediaKey(companyMediaKeyItem?.Id as number));
  ...
  useFocusEffect(
    React.useCallback(() => {
      dispatch(clearCompanyMediaKeyError());
    }, [dispatch]),
  );

  return (
    <View style={[formStyles.form, layoutStyles.parentForm, cmsStyles?.companyMediaKeyForm, style?.form]}>
      ...
      { buttonsTop && (
          <View style={[formStyles.buttons, style?.buttons]}>
            <Button
              ...
              testID={"submit-CompanyMediaKey-form"}
              title={itemExists() ? updateButtonText ? updateButtonText : `Update Company Media Key` : saveButtonText ? saveButtonText : `Save Company Media Key`}
            ...
            {!hideDeleteButton && itemExists() && (
              <Button
                ...
                testID={"deleteCompanyMediaKey"}
                title={deleteButtonText ? deleteButtonText : `Delete Company Media Key`}
        ...
        <Toggle
          ...
          testID={"company-media-key-multiple"}
...
export default CompanyMediaKeyForm;

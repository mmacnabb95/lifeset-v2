...
import { createStreakLeaderBoard, updateStreakLeaderBoard, deleteStreakLeaderBoard, streakLeaderBoardErrorSelector, streakLeaderBoardSelector , clearStreakLeaderBoardError } from "src/redux/domain/features/streakLeaderBoard/collection-slice";
...
const StreakLeaderBoardForm = forwardRef(({
  customTextValues,
  afterCreate,
  ...
  formItem?: Partial<Streakleaderboard>,
  ...
  handleValuesChange?: ({
    ...
    values: Partial<Streakleaderboard>,
    errors: FormikErrors<Streakleaderboard>
  }) => void,
  ...
  const error = useSelector(streakLeaderBoardErrorSelector);
  const streakLeaderBoardItem = useSelector(streakLeaderBoardSelector(formItem?.Id));
  const [currentTextValues, setCurrentTextValues] = useState(defaultTextValues);

  
  const initialValues: Partial<Streakleaderboard> = {
      Company: streakLeaderBoardItem?.Company || formItem?.Company || undefined,
      User: streakLeaderBoardItem?.User || formItem?.User || undefined,
      UserFullName: streakLeaderBoardItem?.UserFullName || formItem?.UserFullName || '',
      MaxStreakThisMonth: streakLeaderBoardItem?.MaxStreakThisMonth || formItem?.MaxStreakThisMonth || undefined,
      Url: streakLeaderBoardItem?.Url || formItem?.Url || '',
      Meta: streakLeaderBoardItem?.Meta || formItem?.Meta || '',
  };
  ...
  const itemExists = () => {
    return streakLeaderBoardItem || (formItem && formItem?.Id && formItem.Id > 0) ? true : false;
  };

  const getItemId = () => {
    if (streakLeaderBoardItem?.Id) return { Id: streakLeaderBoardItem?.Id };
    return {};
  }

  useImperativeHandle(ref, () => (formik));

  const formik = useFormik<Partial<Streakleaderboard>>({
    ...
    onSubmit: async (streakLeaderBoard) => {
      let response:any;
      if (itemExists()) {
        const updateObjectFull = { ...streakLeaderBoard, ...getItemId() } ;
        const updateObjectIgnored = _.omit(updateObjectFull, ["HasVideo", "HasImage", ]);
        response = await dispatch(updateStreakLeaderBoard(updateObjectIgnored));

        if(afterUpdate) afterUpdate({...response, isValid});
      } else {
        response = await dispatch(createStreakLeaderBoard(_.omit(formik.values, ["HasVideo", "HasImage"])));
  ...
  const handleDelete = async () => {
      if (!beforeDelete || await beforeDelete() === true) {
        const response: any = await dispatch(deleteStreakLeaderBoard(streakLeaderBoardItem?.Id as number));
  ...
  useFocusEffect(
    React.useCallback(() => {
      dispatch(clearStreakLeaderBoardError());
    }, [dispatch]),
  );

  return (
    <View style={[formStyles.form, layoutStyles.parentForm, cmsStyles?.streakLeaderBoardForm, style?.form]}>
      ...
      { buttonsTop && (
          <View style={[formStyles.buttons, style?.buttons]}>
            <Button
              ...
              testID={"submit-StreakLeaderBoard-form"}
              title={itemExists() ? updateButtonText ? updateButtonText : `Update Streak Leader Board` : saveButtonText ? saveButtonText : `Save Streak Leader Board`}
            ...
            {!hideDeleteButton && itemExists() && (
              <Button
                ...
                testID={"deleteStreakLeaderBoard"}
                title={deleteButtonText ? deleteButtonText : `Delete Streak Leader Board`}
...
export default StreakLeaderBoardForm;

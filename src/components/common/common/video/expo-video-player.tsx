...
import {
  ...
  ErrorSeverity,
  PlaybackStates,
} from "expo-video-player/dist/constants";
import {
  ErrorMessage,
  TouchableButton,
  deepMerge,
  ...
  styles,
} from "expo-video-player/dist/utils";
import { MaterialIcons } from "@expo/vector-icons";
import { Props, defaultProps } from "expo-video-player/dist/props";
import { useEffect, useRef, useState } from "react";
import React from "react";
import Slider from "@react-native-community/slider";

//copy pasta to fix bugs from https://raw.githubusercontent.com/ihmpavel/expo-video-player/master/lib/index.tsx
//TODO: Raise this issue on GitHub
// issues fixed with playingRefOverride sledge hammer

const VideoPlayer = (tempProps: Props) => {
  const props = deepMerge(defaultProps, tempProps) as Props;
  const playingRefOverride = useRef(false);
  ...
  useEffect(() => {
    ...
    return () => {
      if (playbackInstance) {
        ...
        playingRefOverride.current = false;
      }
    };
  ...
  const togglePlay = async () => {
    ...
    if (playbackInstanceInfo.state !== PlaybackStates.Ended) {
      playingRefOverride.current = !playingRefOverride.current;
    }

    if (controlsState === ControlStates.Hidden) {
      return;
    }
    const shouldPlay = playingRefOverride.current; // playbackInstanceInfo.state !== PlaybackStates.Playing;
    if (playbackInstance !== null) {
      ...
      setPlaybackInstanceInfo({
        ...
        state:
          //   playbackInstanceInfo.state === PlaybackStates.Playing
          playingRefOverride.current === true
            ? PlaybackStates.Playing
            : PlaybackStates.Paused,
      });
      if (shouldPlay) {

...
interface ResourceEditNavOptionsProps {
  ...
  readonly listItem: Workoutdayexercise & {
    summary: { ExerciseName: string; SetCount: number; SetTypeName: string };
  };
}

export const WorkoutDayExerciseEditNav = ({
  navigation,
  params,
  destination,
  text,
      ...
      <View style={[commonStyles.row]}>
        ...
        >
          <Typography
            numberOfLines={2}
            ellipsizeMode="tail"
            text={listItem?.summary?.ExerciseName}
          ...
          <View style={commonStyles.categoryView}>
            <Typography
              ...
              ellipsizeMode="tail"
              text={`${
                listItem?.summary?.SetCount
              } ${listItem?.summary?.SetTypeName?.toLowerCase()} set${
                listItem?.summary?.SetCount > 1 ? "s" : ""
              }`}
              style={{ fontSize: 14 }}
            />
          </View>

...
import { createHabitCompletedRecord, deleteHabitCompletedRecord } from 'src/redux/domain/features/habitCompletedRecord/collection-slice';
...
export const ResourceCheckboxNav = ({
  ...
  const toggleChecked = async () => {
    ...
    try {
      ...
      if (newChecked === true) {
        try {
          await dispatch(createHabitCompletedRecord({
            Habit: source.Id,
          ...
          const result = await dispatch(thunks?.getHabitsByDate({
            user: source.User,
            date: today,
            offset: 0,
            limit: 10000,
          ...
          const todaysHabits = result?.payload?.filter((h: any) => 
            h.Date === today && 
            h.CreatedDate && 
            (!h.DeletedDate || new Date(h.DeletedDate) > new Date(today))
          );
          
          const allCompleted = todaysHabits?.length > 0 && 
        ...
        const todaysCompletion = source?.CompletionOverMonth?.find(
          (c: { habitCompleteRecord?: any }) => {
            if (c.habitCompleteRecord) {
              const completionDate = new Date(c.habitCompleteRecord.HabitCompletedDate);
        ...
        if (todaysCompletion?.habitCompleteRecord?.Id) {
          try {
            await dispatch(deleteHabitCompletedRecord(
              todaysCompletion.habitCompleteRecord.Id
            ));
            
            success = true;
              
            await dispatch(thunks?.getHabitsByDate({
              user: source.User,
              date: today,
              offset: 0,
              limit: 10000,
  ...
  return (
    <>
      ...
      >
        <View style={[commonStyles.row]}>
          ...
          <Pressable
            ...
            style={{
              height: 48,
              width: 48,
              backgroundColor: checked ? '#32CD32' : constants.checkboxBackground,
              borderRadius: 8,
              alignItems: "center",
        ...
        {!rightIcon && (
          <View style={{ width: 50, borderLeftWidth: 1, borderLeftColor: 'rgba(0, 0, 0, 0.05)' }}>
            <Pressable
              onPress={() => {

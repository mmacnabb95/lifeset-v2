...
interface ResourceEditNavOptionsProps {
  ...
  readonly listItem: Workoutexerciseset;
}

export const WorkoutExerciseSetEditNav = ({
  navigation,
  params,
  destination,
  text,
  ...
  const repSummary = () => {
    let weight = "";
    const weightVal = (listItem as any).WeightKg;
    if (weightVal) {
      weight = ` ${weightVal}kg${weightVal > 1 ? "s" : ""}.`;
    }

    if (listItem.RepCount) {
      return `${listItem.RepCount} rep${
        listItem.RepCount > 1 ? "s" : ""
      }.${weight}.`;
    }
    if (listItem.RepTimeSeconds) {
      return `${listItem.RepTimeSeconds}s. `;
    }
    return "";
  };
